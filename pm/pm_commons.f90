module pm_commons
  ...
  !introduced by PH 30/12/2019 to compute the radiative feedback from the sink
  real(kind=8),allocatable,dimension(:)::dtdmsink

  !introduced by PH 7/01/2020 to smooth the accretion rate for the feedback
  !this is in units of 1/sqrt(G rho)
  real(kind=8) :: coef_smooth_accret=1
