...
subroutine set_uold(ilevel)
  ...
  if(myid==1)write(*,*)'time_relaxing',t,t_nograv
  if(poisson .and. (t .gt. t_nograv))then
  if(myid==1)write(*,*)'time_passed',t,t_nograv
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           do igroup=1,ngrp
              ...
              kappa_R = rosseland_ana(d_loc,Tp_loc,Tr_loc,igroup)/scale_kappa
              gradEr_norm2 = 0.0d0
              do idim=1,ndim
...
!              read(*,*)
           ...
           do igroup=1,ngrp
              ...
              do idim=1,ndim
                 ...
                 unew(ind_cell(i),1+idim) = unew(ind_cell(i),1+idim) - dtnew(ilevel)*rosseland_ana(d_loc,Tp_loc,Tr_loc,igroup)/scale_kappa*uold(ind_cell(i),firstindex_er+idim*ngrp+igroup)*scale_v/clight
                 unew(ind_cell(i),5) = unew(ind_cell(i),5) - dtnew(ilevel)*rosseland_ana(d_loc,Tp_loc,Tr_loc,igroup)/scale_kappa*uold(ind_cell(i),firstindex_er+idim*ngrp+igroup)*scale_v/clight*uold(ind_cell(i),1+idim)
...
!             take care betaadbricolo should be used
           ...
           write(*,*) 'local_density', d, eps, ind_cell(i), uold(ind_cell(i),1)
