...
subroutine clump_finder(create_output,keep_alive)
  ...
  if(verbose) write(*,*) 'start sort cells'
  !-----------------------------------------------------------------------
  ...
  if(verbose) write(*,*) 'start count number of density peaks'
  !-----------------------------------------------------------------------
  ...
  if(verbose) write(*,*) 'determine peak-ids'
  !----------------------------------------------------------------------
  ...
  npeaks_max=MAX(40*maxval(npeaks_per_cpu_tot),1000)
  ...
  if(verbose) write(*,*) 'start finding peak patches'
  !---------------------------------------------------------------------
  ...
  if(verbose) write(*,*) 'start allocate peak-patch'
  if(verbose) write(*,*) 'myid pp', myid
  !------------------------------------
  ! Allocate peak-patch property arrays
  !------------------------------------
  call allocate_peak_patch_arrays

  if(verbose) write(*,*) 'start build peak comm'
  if(verbose) write(*,*) 'myid', myid, 'ncpu ',ncpu, 'ncpu^2 ',ncpu*ncpu
  call build_peak_communicator

  if(verbose) write(*,*) 'start saddle point, merging, properties computing'
...
122 rewind(1)
  ...
  !allocate the peak communicator
  !PH add these lines 11/01/2021 because get a segfault when in
  !build_peak_communicator  in OCCIGEN for ncpu > 1000
