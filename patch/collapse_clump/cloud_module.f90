...
!  use hydro_parameters !, ONLY: nvar,boundary_var,gamma,bx_bound,by_bound,bz_bound,turb,dens0,V0
  ...
  real(kind=8)::rho_max_loc,rho_max_all,epot_loc,epot_all,ekin
  ...
  real(dp)::dx_min, fact, Emag,Emag0,enint,radiation_source
     ...
     do ind=1,twotondim
       ...
       do i=1,ngrid
        ...
        if(  xx(i,1) .lt. 2.*dx_min ) then 
           ...
           Ekin= 0.5*(uold(ind_cell(i),2)**2.+uold(ind_cell(i),3)**2.+uold(ind_cell(i),4)**2.)/ uold(ind_cell(i),1)
           uold(ind_cell(i),5) = uold(ind_cell(i),5)  - Emag - Ekin
           !no-inflow
           if(no_inflow) then
              uold(ind_cell(i),2)=min(uold(ind_cell(i),2),0.0d0) 
           endif
           
           uold(ind_cell(i),9) = radiation_source(Tr_floor)/(scale_d*scale_v**2)
           call enerint_eos(uold(ind_cell(i),1),Tr_floor,enint)
           uold(ind_cell(i),nvar) = enint
           uold(ind_cell(i),5)    = enint + uold(ind_cell(i),9)
           ...
           !We re-propagate the imposed conditions for the neighbouring cells /!\ This is necessary for FLD to work
           ...
           Emag = 0.5*(A**2+B**2+C**2)
           uold(ind_cell_vois,9)=radiation_source(Tr_floor)/(scale_d*scale_v**2)
           uold(ind_cell_vois,nvar)=uold(ind_cell(i),nvar)
           uold(ind_cell_vois,5)=enint+uold(ind_cell_vois,9)+Ekin+ Emag
        ...
        if(  xx(i,1) .gt. boxlen-2.*dx_min ) then 
           ...
           uold(ind_cell(i),1) = MAX(uold(ind_cell_vois,1),smallr)
           ...
           !no-inflow
           if(no_inflow) then
              uold(ind_cell(i),2)=max(uold(ind_cell(i),2),0.0d0) 
           endif
        ...
        if(  xx(i,2) .lt. 2.*dx_min ) then 
           ...
           !no-inflow
           if(no_inflow) then
              uold(ind_cell(i),3)=min(uold(ind_cell(i),3),0.0d0) 
           endif
        ...
        if(  xx(i,2) .gt. boxlen-2.*dx_min ) then 
           ...
           !no-inflow
           if(no_inflow) then
              uold(ind_cell(i),3)=max(uold(ind_cell(i),3),0.0d0) 
           endif
           uold(ind_cell(i),9)=radiation_source(Tr_floor)/(scale_d*scale_v**2)
           call enerint_eos(uold(ind_cell(i),1),Tr_floor,enint)
           uold(ind_cell(i),nvar)=enint
        ...
        if(  xx(i,3) .lt. 2.*dx_min ) then 
           ...
           !no-inflow
           if(no_inflow) then
              uold(ind_cell(i),4)=min(uold(ind_cell(i),4),0.0d0) 
           endif
           uold(ind_cell(i),9)=radiation_source(Tr_floor)/(scale_d*scale_v**2)
           call enerint_eos(uold(ind_cell(i),1),Tr_floor,enint)
           uold(ind_cell(i),nvar)=enint
        ...
        if(  xx(i,3) .gt. boxlen-2.*dx_min ) then 
           ...
           !no-inflow
           if(no_inflow) then
              uold(ind_cell(i),4)=max(uold(ind_cell(i),4),0.0d0) 
           endif
           uold(ind_cell(i),9)=radiation_source(Tr_floor)/(scale_d*scale_v**2)
           call enerint_eos(uold(ind_cell(i),1),Tr_floor,enint)
           uold(ind_cell(i),nvar)=enint
...
!#########################################################
!#########################################################
! Routine to change of galilean frame to re_center around
! a given sink
! Written by Ugo Lebreuilly in Autumn 2021
! Greatly inspired from Troel's routine. Many thanks to
! him.
!#########################################################
!#########################################################
!#########################################################
!#########################################################
subroutine change_frame
  Use amr_commons
  use pm_commons
  use hydro_commons
  use radiation_parameters
  ...
  integer ,dimension(1:nvector),save::ind_grid,ind_cell
  real(dp):: vx_frame,vy_frame,vz_frame
  integer :: ilevel,ncache,iskip,i,igrid,ind,ngrid,isink,ivar
  real(dp):: etot,ekin_new,ekin_old,d,u,v,w


  if(nsink>=isink_refine) then
     vx_frame=vsink(isink_refine,1)
     vy_frame=vsink(isink_refine,2)
     vz_frame=vsink(isink_refine,3)
     
     if(myid.eq.1) then
        
        write(*,*)  'Re-centering to the sink co-moving frame'
        write(*,*)  "Vx = ",Vx_frame
        write(*,*)  "Vy = ",Vy_frame
        write(*,*)  "Vz = ",Vz_frame
        ...
        do igrid=1,ncache,nvector
           ...
           do ind=1,twotondim
              ...
              do i=1,ngrid
                 ...
                 ekin_old= 0.5*(uold(ind_cell(i),2)**2+uold(ind_cell(i),3)**2+uold(ind_cell(i),4)**2)/d
                 uold(ind_cell(i),2)= uold(ind_cell(i),2)-d*vx_frame
                 uold(ind_cell(i),3)= uold(ind_cell(i),3)-d*vy_frame
                 uold(ind_cell(i),4)= uold(ind_cell(i),4)-d*vz_frame
                 ekin_new= 0.5*(uold(ind_cell(i),2)**2+uold(ind_cell(i),3)**2+uold(ind_cell(i),4)**2)/d
                 uold(ind_cell(i),5)=etot+ekin_new-ekin_old
              end do
           enddo
        end do
     end do
     do ilevel=levelmin,nlevelmax
        call make_virtual_fine_dp(uold(1,2),ilevel)
        call make_virtual_fine_dp(uold(1,3),ilevel)
        call make_virtual_fine_dp(uold(1,4),ilevel)
        call make_virtual_fine_dp(uold(1,5),ilevel)
        if(simple_boundary)call make_boundary_hydro(ilevel)
     end do
     do isink=1,nsink
        vsink(isink,1)=vsink(isink,1)-vx_frame
        vsink(isink,2)=vsink(isink,2)-vy_frame
        vsink(isink,3)=vsink(isink,3)-vz_frame
     end do


  endif

end subroutine change_frame
!#########################################################
!#########################################################
!#########################################################
!#########################################################
