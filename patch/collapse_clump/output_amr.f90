...
subroutine dump_all
  ...
  if(nstep_coarse==nstep_coarse_old.and.nstep_coarse>0.and.new_ncpu==0)return
  if(nstep_coarse==0.and.nrestart>0.and.new_ncpu==0)return
...
!   if(ndim>1)then
...
#endif
     if(myid==1.and.print_when_io) write(*,*)'Stsart backup header'
     ...
     if(synchro_when_io) call MPI_BARRIER(MPI_COMM_WORLD,info)
#endif
     if(myid.le.new_ncpu.or.new_ncpu.eq.0) then
