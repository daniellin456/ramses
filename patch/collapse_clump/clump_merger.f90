...
subroutine compute_clump_properties(xx)
...
#if NDUST>0
  use hydro_commons, ONLY:ndust,firstindex_ndust
#endif  
  use clfind_commons
  implicit none
#ifndef WITHOUTMPI
  ...
  !----------------------------------------------------------------------------
  integer::ipart,i,grid,peak_nr,ilevel,global_peak_id,ipeak,plevel
  real(dp)::zero=0.
...
real(dp) :: sum_dust
  ...
  do ipart=1,ntest
     ...
     if (global_peak_id /=0 ) then
        ...
        enddo
#endif
        call soundspeed_eos((1.0d0-sum_dust)*d,eint,cs)
...
!  real(dp),dimension(1:ncpu,1:ncpu)::npeak_alltoall
!  real(dp),dimension(1:ncpu,1:ncpu)::npeak_alltoall_tot
  integer,dimension(1:ncpu)::ipeak_alltoall


  if(verbose) write(*,*) 'enter in build peak comm'
  if(verbose) write(*,*) 'npeaks, hfree, myid, ncpu ',npeaks,hfree,myid,ncpu
  ...
  if(verbose) write(*,*) 'start mpi_allreduce'
  if(verbose) write(*,*) 'ncpu^2 ',ncpu*ncpu

  call MPI_ALLREDUCE(npeak_alltoall,npeak_alltoall_tot,ncpu*ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)

!  call MPI_ALLREDUCE(npeak_alltoall,npeak_alltoall_tot,ncpu*ncpu,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
...
!!$  end if
  ...
  if(verbose) write(*,*) 'check allocation'
  ...
  if(verbose) write(*,*) 'fill communicator'
...
!     peak_send_cnt(icpu)=int(npeak_alltoall(myid,icpu))
!     peak_recv_cnt(icpu)=int(npeak_alltoall(icpu,myid))
...
!        peak_send_oft(icpu+1)=peak_send_oft(icpu)+int(npeak_alltoall(myid,icpu))
!        peak_recv_oft(icpu+1)=peak_recv_oft(icpu)+int(npeak_alltoall(icpu,myid))
     endif
  end do

  if(verbose) write(*,*) 'check allocation buf'
  if(allocated(peak_send_buf))then
     deallocate(peak_send_buf,peak_recv_buf)
  endif

  if(verbose) write(*,*) 'allocate buf'
  ...
  if(verbose) write(*,*) 'mpi_alltoallv'
