subroutine adaptive_loop
  ...
  !RE-DUMP if we want to reduce the number of cpu
  if(new_ncpu>0) then
     
     old_ncpu=ncpu
     new_ncpu_goal=new_ncpu
     new_ncpu=old_ncpu
     if(myid==1)write(*,*) ' Changing the number of cpu from', old_ncpu, ' to ', new_ncpu_goal
     call load_balance
     do while (new_ncpu .gt. new_ncpu_goal)
        new_ncpu=new_ncpu-reduce_cpu_per_load
        if(new_ncpu.lt.new_ncpu_goal) new_ncpu=new_ncpu_goal
        if(myid==1) write(*,*) 'Calling load balance for ncpu =', new_ncpu
        call load_balance
     end do
     if(myid==1) write(*,*) 'A last load balancing to clean everything'
     call load_balance
     call dump_all
     if(myid==1) write(*,*) 'A new output with', new_ncpu,' cpu has been dumped'
     call clean_stop
     !YOU DID IT
  endif
