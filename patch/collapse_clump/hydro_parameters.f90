...
! Cosmic rays energy groups
  ...
  real(dp),dimension(1:MAXLEVEL)::jeans_refine_2=-1.0
  real(dp),dimension(1:ndim)::center_refine = 0.5! in code units

  real(dp) :: radius_refine = 0.1 ! in code units
  real(dp),dimension(1:ndim):: vframe_zoom= 0.0d0 ! Sink velocity to change frame when rezooming
  logical:: rezoom=.false. ! Set to true for rezoomed run
  logical:: frig_bound=.false.
  ...
  real(dp):: rho_threshold_smol=1d-17                    ! Density threshold to solve for the growth (in c.g.s)
  ...
  logical :: spitzer_charging=.false.                    ! Computes the grain charge as in Spitzer 1941
  ...
  real(dp) :: smalleps = 1d-10                           ! Minimum dust ratio per dust species
  ...
  real(dp) :: n_thresold_dust=1d5                        ! Threshold under which the dust density is divided by /reduce_dfrac
