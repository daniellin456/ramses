subroutine hydro_flag(ilevel)
  ...
  if(    err_grad_d==-1.0.and.&
       ...
       & jeans_refine_2(ilevel)==-1.0 )return
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if(poisson.and.jeans_refine_2(ilevel)>0.0)then
        ...
        call jeans_length_refine_2(ind_cell,xx,ok,ngrid,ilevel)
     endif
...
subroutine jeans_length_refine(ind_cell,ok,ncell,ilevel)
 ...
 real(dp)::sum_dust
  ...
  do i=1,ncell
     ...
     tempe2 = tempe
     if(iso_jeans .and. (dens*scale_d .lt. rho_star).and.(dens*scale_d .gt. rho_disk)) then
...
!#####################################################################
!#####################################################################
subroutine jeans_length_refine_2(ind_cell,xx,ok,ncell,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use radiation_parameters, ONLY: mu_gas
  !use cloud_module, ONLY:mH
  use poisson_commons
  use cooling_module, ONLY: twopi,mh
  use units_commons
  implicit none
  integer::ncell,ilevel
#if NENER>0
  ...
  !-------------------------------------------------
  integer::i,indi
  real(dp)::lamb_jeans,tail_pix,pi,n_jeans,rr
  real(dp)::dens,tempe,emag,etherm,factG
  real(dp)::iso_etherm,iso_cs,iso_cs2,rho_star,rho_iso,tempe2
 ...
 real(dp)::sum_dust
  ...
  factG=1.0d0
  if(cosmo)factG=3d0/8d0/pi*omega_m*aexp
  n_jeans = jeans_refine_2(ilevel)
  ! compute the size of the pixel
  tail_pix = boxlen / (2.d0)**ilevel
  do i=1,ncell
     indi = ind_cell(i)
     rr=sqrt((xx(i,1) -xsink(isink_refine,1))**2.+(xx(i,2) -xsink(isink_refine,2))**2.+(xx(i,3) -xsink(isink_refine,3))**2.0)/radius_refine
     if(rr<=1.0d0) then
        ! the thermal energy
        dens = max(uold(indi,1),smallr)
        etherm = uold(indi,5)
  ...
  ! the temperature
     ...
     if(iso_jeans .and. (dens*scale_d .lt. rho_star)) then
        ...
        ! the Jeans length must be smaller
        ! than n_jeans times the size of the pixel
        ok(i) = ok(i) .or. ( n_jeans*tail_pix >= lamb_jeans.and.rr<1.0d0)
     endif
  end do
