module amr_parameters
...
#endif
#if NDUST>0  
  logical::dust_diffusion=.false. ! Enable dust diffusion (Lebreuilly et al, 2019 solver)
#endif
  ! Mesh parameters
  ...
  integer,dimension(1:MAXLEVEL):: weight_cost= 1
...
!Cosmic rays related variables
  ...
  integer:: isink_refine=-1 ! Index of the sink around which rezooming
  logical:: galilean_transfo=.false. !Perform a galilean transfo at each coarse dt
