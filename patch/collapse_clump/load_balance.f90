...
#if NDUST>0
  use hydro_commons, ONLY: v_dust,ndust
#endif 
...
subroutine cmp_new_cpu_map
...
#endif
  old_ncpu=ncpu
  if(new_ncpu>0)ncpu=new_ncpu
  if(new_ncpu>0)ndomain=new_ncpu*overload
  ! Local constants
  nxny=nx*ny
  ...
  if(cost_weighting)then
     ...
     niter_cost(levelmin:nlevelmax)=weight_cost(levelmin:nlevelmax)
  endif
  ...
  ! End loop over levels
  ncpu=old_ncpu
  if(new_ncpu>0)ndomain=ncpu*overload
...
#if NDIM>1
  real(kind=8)::xx,yy,xc,yc,atan3,xshift=0.125d0
#endif
#if NDIM>2
  real(kind=8)::zz,zc
#endif
...
subroutine cmp_minmaxorder(x,order_min,order_max,dx,nn)
  ...
  real(dp)::dxx,dxmin
#if NDIM>1
  real(dp)::theta1,theta2,theta3,theta4,xshift=0.125d0,atan3
  real(kind=8)::xx,yy,xc,yc,xx1,xx2,yy1,yy2,xx3,yy3
#endif
#if NDIM>2
  real(kind=8)::zz,zc
#endif
...
#if NDIM>1
  if(ordering=='angular')then ! Neil version
     ! Angular domain decomposition
     dxx=0.5d0*dx
     !      xc=boxlen/2.
     !      yc=boxlen/2.
...
do idim=1,ndim
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=real(v_dust(iskip2+igrid,idust,idim),kind=qdp)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     v_dust(iskip2+igrid,idust,idim)=real(hilbert_key(igrid),kind=8)
  end do
end do
end do
end do
