...
!PH modified 15/03/2020
     le = nEnergy-1
!     stop
  endif

  ie = floor(le)
  if  (ir < 1 .or. ie < 1 .or. ir>nrho-1 .or. ie>nEnergy-1) then
     write(*,*) 'inter_pressure hors limite i,ir,ie,rho,Enint = ',ir,ie,rho ,Enint , nrho, nEnergy
!     ir=1.0d0
!     if (ie < 1 ) ie=1.0d0
     ...
     if (ir < 1 .or. ir>nrho-1 .or. ie>nEnergy-1) stop 
!     stop
  endif
...
!     write(*,*) 0.25*(P_eos(ir,ie+2) + P_eos(ir-1,ie+1) +   P_eos(ir+1,ie+1) +  P_eos(ir,ie+1))
!     stop
!  endif

  !PH add a fix when Eint is too low 13/04/2018
     if (ie < 1 ) then 
        Peos = (gamma-1.d0)*Enint_temp
        write(*,*) Peos *(scale_d*scale_v**2)
     endif

!     call enerint_eos(rho_temp,Tr_floor,e_prim)

!     E = max(Enint_temp,e_prim)
...
double precision function barotrop1D(rhon)
  ...
  !PH includes the more complete eos
  real(dp)::gamma2,n_star2
  gamma2=7./5.
  n_star2=30.*n_star

  if(analytical_barotrop)then
     barotrop1D = Tr_floor * ( 1.0d0 + (rhon/n_star)**(gamma-1.0d0) / (1.+(rhon/n_star2)**(gamma-gamma2)) )
  else
     inp=rhon ! in g.cc
