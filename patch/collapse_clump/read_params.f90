subroutine read_params
  ...
  namelist/run_params/clumpfind,cosmo,pic,sink,lightcone,poisson,hydro,rt,verbose,debug &
       ...
       & ,bisec_tol,static,geom,overload,cost_weighting,aton,nrestart_quad,restart_remap,weight_cost &
#if NIMHD==1
       ...
       & ,x_load_balance,y_load_balance,z_load_balance,angular_auto_center,angular_auto_min_rho,galilean_transfo
  namelist/output_params/noutput,foutput,fbackup,aout,tout &
       & ,tend,delta_tout,aend,delta_aout,gadget_output,walltime_hrs,minutes_dump,write_conservative,new_ncpu,reduce_cpu_per_load
...
81 continue
  ...
  do i=nlevelmax,levelmin,-1
     ...
     weight_cost(i)=weight_cost(i-levelmin+1)
  end do
  do i=1,levelmin-1
     nexpand   (i)= 1
     nsubcycle (i)= 1
