...
!  acc_rate(1:nsink)=delta_mass(1:nsink)/dtnew(levelmin)
!PH 23/12/2019 - debug the instantaneous acc_rate
  call setup_accretion_rate
...
!################################################################
!################################################################
!!This routine verify whether the accretion rate onto the sink 
!!particles need to be updated. The accretion rate is calculated based 
!!on a time interval equal to t_smooth_accret
subroutine setup_accretion_rate
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
...
integer::isink
real(dp):: t_smooth_accret
  
  !the smoothing time for accretion is taken as the freefall time of 
  !the sink density threshold
  if(d_sink > 0.) then 
    t_smooth_accret = 1./sqrt(d_sink) * coef_smooth_accret
  else
    !this is an arbitray choice that must be improved
    !about 80 yr with the usual units
    t_smooth_accret = 1.d-6
  endif

  do isink=1,nsink
     if(t-dtdmsink(isink) .gt. t_smooth_accret) then
        acc_rate(isink) = dmfsink(isink) / (t-dtdmsink(isink))
        dmfsink(isink)=0.
        dtdmsink(isink)=t
     endif
  enddo

end subroutine setup_accretion_rate
!################################################################
!################################################################
!################################################################
!################################################################
...
!!$        delta_mass(isink)=delta_mass(isink)+delta_mass_all(isink)
     ...
     !PH 23/12/2019 PH devu acc_rate
!     acc_rate(1:nsink)=acc_rate(1:nsink)+delta_mass(1:nsink)/dtnew(ilevel)
        ...
        if(ok(j,ind))then ! Only volumes at the current level
           ...
           if (on_creation)then
              if (new_born(isink))then
                 ...
                 m_acc=mseed(isink)*weight/volume*d/density
              else
...
#endif                    
                    call soundspeed_eos(d*(1.0d0-sum_dust),eint,c2)
                    c2=c2**2
                    d_jeans=c2*3.1415926/(4.0*dx_loc)**2/factG
                    d_floor=d_jeans
                 endif
                 m_acc=c_acc*weight*(d-d_floor)
                 !PH 31/01/2020 Try to avoid too strong accretion that makes the timesteps to shrink
                 if( uold(indp(j,ind),1) .gt. 1.1*unew(indp(j,ind),1) ) m_acc=0.
              end if
           end if
           ...
           if (8*m_acc/vol_loc>d .and. (.not. on_creation))then 
              write(*,*)'====================================================='
              write(*,*)'DANGER of neg density :-( at location'
              write(*,*)xx(j,1:3,ind)
              write(*,*)'due to',isink
              write(*,*)m_acc/vol_loc,d/d_sink,density/d_sink
...
!PH 14/03/2020 remove the delayed accretion luminosity (there is no justification for this and it creates peak in the IMF) 
              if(star_mass*scale_m/Msun .gt. 7.1d-2)then
!              if(star_mass*scale_m/Msun .gt. 3.d-2)then
!              if(star_mass*scale_m/Msun .gt. 0.)then
...
!PH 25/11/2019 :take care this line was commented and the following was on
!but acc_rate is not properly calculated...
...
!              if(dtdmsink(isink) .ne. 0) then
!                 acc_lum(isink) = facc_star_lum*star_mass*facc_star*dmfsink(isink)/dtdmsink(isink)/radius
!                 acc_rate(isink) = dmfsink(isink)/dtdmsink(isink)
!              endif
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           do i=1,nnew
              ...
              d=d-delta_d
!              e=e*d
              call enerint_eos(d*(1.0d0-sum_dust),temp,e)
  ...
  do isink=1,nsink
     if(oksink_all(isink)==1)then
        ...
        !introduced by PH 30/12/2019 to follow the radiative feedback from the sinks
        dtdmsink(isink) = tsink(isink)
...
! compute gradient
  ...
  do i=1,3
     do j=1,3
        ...
        if(numerator>0) displacement(i)=displacement(i)-numerator/(det+10.*numerator*smallreal)
     end do
  end do
...
subroutine merge_smbh_sink
  ...
  integer::j,isink,jsink,i,nx_loc,mergers
  real(dp)::dx_loc,scale,dx_min,rr,rmax2,rmax,mcom,v1_v2,factG
  logical::merge
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(dp),dimension(1:3)::xcom,vcom,lcom
...
subroutine read_sink_params()
  ...
  namelist/sink_params/n_sink,rho_sink,d_sink,accretion_scheme,merging_timescale,jeans_accretion,&
       ...
       clump_core,verbose_AGN,T2_AGN,v_AGN,cone_opening,mass_halo_AGN,mass_clump_AGN,feedback_scheme,coef_smooth_accret
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
