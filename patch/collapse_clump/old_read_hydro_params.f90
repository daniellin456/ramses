subroutine read_hydro_params(nml_ok)
  ...
  !--------------------------------------------------
  namelist/init_params/filetype,initfile,multiple,nregion,region_type,rezoom,boxlen &
  ...
  namelist/refine_params/x_refine,y_refine,z_refine,r_refine &
       & ,a_refine,b_refine,exp_refine,jeans_refine,jeans_refine_2,center_refine,radius_refine,mass_cut_refine &
       & ,iso_jeans,Tp_jeans &
       ...
       & ,interpol_var,interpol_type,sink_refine,interpol_mag_type,vframe_zoom,isink_refine
  namelist/boundary_params/nboundary,bound_type &
       & ,ibound_min,ibound_max,jbound_min,jbound_max &
       & ,kbound_min,kbound_max &
       & ,d_bound,u_bound,v_bound,w_bound,p_bound,frig_bound &
#if NENER>NGRP
       & ,prad_bound &
#endif
#if NGRP>0
...
!       & ,rsink_max,msink_max,merge_stars &
  ...
  namelist/dust_params/grain_size, grain_dens, K_dust, K_drag,dust_diffusion_test,slope_dust,dust_ratio,mrn, size_min, size_max, mrn_index&
       ...
       &,dust_growth,smoluchowski,stepinski,charged_dust_dynamics,v_frag,z_dust, spitzer_charging,ndust_max_mrn, rho_threshold_smol,n_thresold_dust
#endif  
  ! fin modif nimhd
...
109 continue
  ...
  if(npscal<ndust)then
   ...
   if(mrn.eqv..true.) then
      ...
      call size_dust_interface(l_grain_interface)
     ...
     do idust=1,ndust
        m_grain(idust)= 4./3.*pi*d_grain(idust)*l_grain(idust)**3.0
     end do
     do idust=1,ndust+1
        l_grain_interface(idust) =  l_grain_interface(idust)/scale_l
     end do
     do idust=1,ndust+1
        m_grain_interface(idust)= 4./3.*pi*d_grain(1)*l_grain_interface(idust)**3.0 ! Dirty d_grain(1) :( TODO add one bin or not (bro)
     end do
     
#if GROWTH==1
     allocate(tabhc(1:ndust))
     allocate(tabflux(1:ndust,1:ndust,1:ndust))
     tabhc=0.0d0
     tabflux=0.0d0
     call grain_growth_init(ndust,m_grain_interface/scale_m,m_grain/scale_m,tabflux,tabhc)
     if(myid.eq.1)print *,  'Growth initialization complete'
#endif
...
call clean_stop
  ...
  if(.not. rezoom)call calc_boxlen
...
!      open(14,file='verif.dat')
  ...
  if(use_nonideal_mhd)then
     if(use_res==1)then
        open(42,file='res_sig.dat', status='old')
        read(42,*) nchimie
        allocate(resistivite_chimie_res(8,nchimie))
        do i=1,nchimie
           read(42,*)resistivite_chimie_res(:,i)
        end do
        close(42)
