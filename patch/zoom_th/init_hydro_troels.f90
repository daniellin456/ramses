subroutine init_hydro
  ...
  real(dp)::old_scale_l,old_scale_d,old_scale_t,old_scale_v
  ...
  old_scale_d=scale_d
  old_scale_l=scale_l
  old_scale_t=0.428566330762690E+14
  old_scale_v=old_scale_l/old_scale_t
  !------------------------------------------------------
...
!!$     if(.not.(neq_chem.or.rt) .and. nvar2.ne.nvar+3)then
           ...
           if(ncache>0)then
              ...
              do ind=1,twotondim
                 ...
                 do ivar=1,4
                    ...
                    if(ivar==1)then ! Read density
                       ...
                       if(write_conservative) then ! Read momentum field
                          do i=1,ncache
                             uold(ind_grid(i)+iskip,ivar)=xx(i)*old_scale_v/scale_v
                          end do
                       else ! Read velocity field
                          do i=1,ncache
                             uold(ind_grid(i)+iskip,ivar)=xx(i)*max(uold(ind_grid(i)+iskip,1),smallr)*old_scale_v/scale_v
                          end do
                       endif
                    end if
                 end do
                 ...
                 if(write_conservative) then
                    ...
                    do i=1,ncache
                       uold(ind_grid(i)+iskip,5)=xx(i)*(old_scale_v/scale_v)**2
                    enddo
                 else
                    read(ilun)xx ! Read pressure
                    if(.not.eos) then
                       do i=1,ncache
                          e=xx(i)/(gamma-1d0)*(old_scale_v/scale_v)**2
                          d=max(uold(ind_grid(i)+iskip,1),smallr)
                          u=uold(ind_grid(i)+iskip,2)/d!*old_scale_v/scale_v
                          v=uold(ind_grid(i)+iskip,3)/d!*old_scale_v/scale_v
                          w=uold(ind_grid(i)+iskip,4)/d!*old_scale_v/scale_v
                          A=0.5*(uold(ind_grid(i)+iskip,6)+uold(ind_grid(i)+iskip,nvar+1))!*old_scale_v/scale_v
                          B=0.5*(uold(ind_grid(i)+iskip,7)+uold(ind_grid(i)+iskip,nvar+2))!*old_scale_v/scale_v
                          C=0.5*(uold(ind_grid(i)+iskip,8)+uold(ind_grid(i)+iskip,nvar+3))!*old_scale_v/scale_v
                          uold(ind_grid(i)+iskip,5)=e+0.5*d*(u**2+v**2+w**2)+0.5*(A**2+B**2+C**2)
                          uold(ind_grid(i)+iskip,nvar)=e
                       end do
                    endif
                 endif
