...
module krome_commons
  ...
  !  KROME 14.08.dev on 2019-10-18 15:08:02
  !  Changeset 484a75f
...
contains
  ...
  function coe(n)
    ...
    kh21 = 8.125d-8*T**(-0.5d0)*exp(-5.2d4*invT)*(1.d0-exp(-6d3*invT))
    ncr21 = 1d1**(4.845d0-1.3d0*log10(T*1d-4)+1.62d0*(log10(T*1d-4))**2)
    a21 = 1.d0/(1.d0+(Hnuclei/ncr21))
    ...
    !H2 + H+ -> H2+ + H
    if(Tgas.GE.1.d2 .and. Tgas.LE.3.d4) then
      k(14) = small + (sumsav&
          *exp(-asav*invT))
    end if

    !H2 + E -> H + H-
    k(15) = small + (3.55d1*Tgas**(-2.28)&
        *exp(-46707.&
        /Tgas))
    ...
    !H2 + H -> H + H + H
    k(17) = small + (dissH2_Martin96(n,Tgas))
    ...
    !H- + H+ -> H + H
    if(Tgas.GE.1d1 .and. Tgas.LE.1d5) then
      k(21) = small + ((2.96d-6&
          /sqrt(Tgas)-1.73d-9+2.50d-10&
          *sqrt(Tgas)-7.77d-13*Tgas))
    end if
    ...
    if(Tgas.LE.1d4) then
      k(23) = small + (1.d6&
          *(4.2278d-14-2.3088d-17*Tgas+7.3428d-21*Tgas**2-7.5474d-25&
          *Tgas**3+3.3468d-29*Tgas**4-5.528d-34*Tgas**5))
    end if

    !H2+ + H- -> H + H2
    k(24) = small + (5.d-7*sqrt(1.d2*invT))
  ...
  subroutine load_arrays()
    ...
    arr_r1(1:27) = (/3,6,6,4,7,7,7,9,3,2,3,3,8,5,5,5,5,2,2,2,2,2&
        ,8,8,3,5,5/)
    arr_r2(1:27) = (/1,1,1,1,1,1,1,1,1,3,6,6,3,6&
        ,1,1,3,1,3,3,6,6,1,2,3,3,5/)
    arr_r3(1:27) = (/13,13,13,13&
        ,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13&
        ,3,3,13/)
    arr_p1(1:27) = (/6,3,3,7,4,4,9,7,2,5,8,8,5,8,3,3&
        ,3,3,3,3,3,8,3,3,5,5,3/)
    arr_p2(1:27) = (/1,13,13,1,13,13&
        ,1,13,13,1,13,13,6,3,2,3,3,1,3,3,3,1,3,5,3,5&
        ,3/)
    arr_p3(1:27) = (/1,13,13,1,13,13,1,13,13,13,13,13,13&
        ,13,13,1,3,1,1,1,13,13,13,13,13,13,5/)
...
contains
  ...
  subroutine fex(neq,tt,nin,dn)
    ...
    dn(idx_E) = &
        ...
        -k(15)*n(idx_H2)*n(idx_E) &
        -k(16)*n(idx_H2)*n(idx_E) &
        +k(16)*n(idx_H2)*n(idx_E) &
        -k(18)*n(idx_Hk)*n(idx_E) &
        +2.d0*k(18)*n(idx_Hk)*n(idx_E) &
        +k(19)*n(idx_Hk)*n(idx_H) &
        +k(20)*n(idx_Hk)*n(idx_H) &
        +k(22)*n(idx_Hk)*n(idx_Hj) &
        -k(23)*n(idx_H2j)*n(idx_E)
    ...
    dn(idx_Hk) = &
        ...
        -k(19)*n(idx_Hk)*n(idx_H) &
        -k(20)*n(idx_Hk)*n(idx_H) &
        -k(21)*n(idx_Hk)*n(idx_Hj) &
        -k(22)*n(idx_Hk)*n(idx_Hj) &
        -k(24)*n(idx_H2j)*n(idx_Hk)
    ...
    dn(idx_H) = &
        ...
        +k(15)*n(idx_H2)*n(idx_E) &
        +2.d0*k(16)*n(idx_H2)*n(idx_E) &
        -k(17)*n(idx_H2)*n(idx_H) &
        +3.d0*k(17)*n(idx_H2)*n(idx_H) &
        +k(18)*n(idx_Hk)*n(idx_E) &
        -k(19)*n(idx_Hk)*n(idx_H) &
        +2.d0*k(19)*n(idx_Hk)*n(idx_H) &
        -k(20)*n(idx_Hk)*n(idx_H) &
        +2.d0*k(20)*n(idx_Hk)*n(idx_H) &
        +2.d0*k(21)*n(idx_Hk)*n(idx_Hj) &
        +2.d0*k(23)*n(idx_H2j)*n(idx_E) &
        +k(24)*n(idx_H2j)*n(idx_Hk) &
        -3.d0*k(25)*n(idx_H)*n(idx_H)*n(idx_H) &
        +k(25)*n(idx_H)*n(idx_H)*n(idx_H) &
        -2.d0*k(26)*n(idx_H2)*n(idx_H)*n(idx_H) &
  ...
  subroutine jes(neq, tt, n, j, ian, jan, pdj)
    ...
    elseif(j==3) then
      ...
      pdj(3) =  &
          ...
          -k(1)*n(idx_E)  &
          -k(11)*n(idx_Hj)  &
          -4.d0*k(26)*n(idx_H2)*n(idx_H)  &
          -k(20)*n(idx_Hk)  &
          +3.d0*k(25)*n(idx_H)*n(idx_H)  &
          +2.d0*k(19)*n(idx_Hk)  &
          -k(17)*n(idx_H2)  &
          -9.d0*k(25)*n(idx_H)*n(idx_H)  &
          +2.d0*k(20)*n(idx_Hk)
      pdj(5) =  &
          +4.d0*k(26)*n(idx_H2)*n(idx_H)  &
          +k(10)*n(idx_Hk)  &
          +k(13)*n(idx_H2j)  &
          -2.d0*k(26)*n(idx_H2)*n(idx_H)  &
          +3.d0*k(25)*n(idx_H)*n(idx_H)  &
          -k(17)*n(idx_H2)
      pdj(6) =  &
          +k(1)*n(idx_E)  &
...
module krome_user
  ...
  integer,parameter::krome_nrea=27
