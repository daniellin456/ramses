...
subroutine create_sink
  ...
  !acc_rate(1:nsink)=delta_mass(1:nsink)/dtnew(levelmin) !# commented by FL to replace with PH routine
...
111 format('   Entering setup_accretion_rate for level ',I2)
...
!              print*,'Acc_lum',facc_star_lum,acc_lum(isink),acc_rate(isink)!star_mass,(t-tsink(isink)),radius!acc_rate(isink)/radius
...
subroutine radiative_feedback_sink(ilevel)
  ...
  use radiation_parameters,only:stellar_photon,aR,sinks_opt_thin
  use units_commons
#ifdef RT
