...
!-------------------------------------------------------------------------
  ...
  lambda=MIN(1d2,157809.1d0/T)
  tbl_beta_HI%rates(iT)  = 5.85d-11 * sqrt(T) / f * exp(-lambda)!-157809.1d0/T)
  tbl_beta_HI%primes(iT) = (hf+157809.1d0/T)                             &
                         * log(10d0) * tbl_beta_HI%rates(iT)
  lambda=MIN(1d2,285335.4d0/T)

  ! Collisional ionization rate [cm3 s-1] of HeI (Maselli&'03)------------
  tbl_beta_HeI%rates(iT)  = 2.38d-11 * sqrt(T) / f * exp(-lambda)!285335.4d0/T)
  tbl_beta_HeI%primes(iT) = (hf+285335.4d0/T)                            &
                          * log(10d0) * tbl_beta_HeI%rates(iT)
  lambda=MIN(1d2,631515.d0/T)
  ! Collisional ionization rate [cm3 s-1] of HeII (Maselli&'03)-----------
  tbl_beta_HeII%rates(iT)  = 5.68d-12 * sqrt(T) / f * exp(-lambda)!-631515.d0/T)
  ...
  lambda=MIN(1d2,157809.1d0/T)
  tbl_cr_ci_HI%rates(iT)  = 1.27d-21 * sqrt(T) / f * exp(-lambda)!)157809.1/T)
  tbl_cr_ci_HI%primes(iT) = (hf+157809.1/T)                              &
                          * log(10d0) * tbl_cr_ci_HI%rates(iT)

  lambda=MIN(1d2,1285335.4d0/T)
  tbl_cr_ci_HeI%rates(iT)  = 9.38d-22 * sqrt(T) / f * exp(-lambda)!285335.4/T)
  tbl_cr_ci_HeI%primes(iT) = (hf+285335.4/T)                             &
                           * log(10d0) * tbl_cr_ci_HeI%rates(iT)

  lambda=MIN(1d2,631515.d0/T)
  tbl_cr_ci_HeII%rates(iT)  = 4.95d-22 * sqrt(T) / f * exp(-lambda)!631515. /T)
  tbl_cr_ci_HeII%primes(iT) = (hf+631515.0/T)                            &
                            * log(10d0) * tbl_cr_ci_HeII%rates(iT)

  ! Collisional excitation cooling from Cen'92
  lambda=MIN(1d2,118348.d0/T)
  tbl_cr_ce_HI%rates(iT)  = 7.5d-19 / f * exp(-lambda)!118348./T)
  tbl_cr_ce_HI%primes(iT) = (118348./T - 0.5d0 * sqrt(T5) / f )          &
                          * log(10d0) * tbl_cr_ce_HI%rates(iT)

  lambda=MIN(1d2,13179.d0/T)
  tbl_cr_ce_HeI%rates(iT)  = 9.10d-27 * T**(-0.1687) / f * exp(-lambda)!13179./T)
  tbl_cr_ce_HeI%primes(iT) = (13179./T - 0.1687 - 0.5d0 * sqrt(T5) / f)  &
                           * log(10d0) * tbl_cr_ce_HeI%rates(iT)

  lambda=MIN(1d2,473638.d0/T)
  tbl_cr_ce_HeII%rates(iT) = 5.54d-17 * T**(-0.397)  / f * exp(-lambda)!473638./T)
  ...
  lambda=MIN(1d2,470000.d0/T)
  f = 1.24d-13*T**(-1.5d0)*exp(-lambda)!470000.d0/T)
  lambda=MIN(1d2,94000.d0/T)
  tbl_cr_die%rates(iT) = f*(1.D0+0.3d0*exp(-lambda))!94000.d0/T))
