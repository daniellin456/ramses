subroutine init_sink
...
#endif
  integer::i, idim, isink, nsinkold
  integer::ilun
  real(dp)::xx1,xx2,xx3,vv1,vv2,vv3,mm1,ll1,ll2,ll3,accr,accl,intl,ttf,agg
  ...
  !introduced by PH 30/12/2019 to compute the radiative feedback around sink
  allocate(dtdmsink(1:nsinkmax))
  dtdmsink=0.0
  ...
  if(nrestart>0)then
     ...
     if(nsink>0)then
        ...
        !to be re-introduced
        !!ADDED by PH 09/2013
        read(ilun)xdp ! Read sink mass accreted since last time period
        dmfsink(1:nsink)=xdp
        ...
        !!ADDED by PH 30/12/2019
        !!to compute radiation feedback
        !! take care to comment this if you want to restart old runs
        read(ilun)xdp ! Read sink time since last time accretion has been computed
        dtdmsink(1:nsink)=xdp
...
!        read(ilun)ncloud_sink
  ...
  if (ic_sink)then
     ...
     eof=.false.
     do
        read(10,*,end=102)mm1,xx1,xx2,xx3,vv1,vv2,vv3,ll1,ll2,ll3,accr,accl,agg,intl,ttf
        nsink=nsink+1
        nindsink=nindsink+1
        idsink(nsink)=nindsink
        msink(nsink)=mm1!/(scale_m/Msun)
        ...
        lsink(nsink,2)=ll2
        lsink(nsink,3)=ll3
        tsink(nsink)=agg/(scale_t/year)
        int_lum(nsink)=intl/(scale_d*scale_l**3*scale_v**2/scale_t/Lsun)
        acc_lum(nsink)=accl/(scale_d*scale_l**3*scale_v**2/scale_t/Lsun)
        acc_rate(nsink)=accr/(scale_m/Msun/(scale_t)*year)
        Teff_sink(nsink)=ttf
        new_born(nsink)=.true.
     end do
102  continue
