...
subroutine flag_formation_sites
  ...
  do j=npeaks,1,-1
     ...
     if (smbh)then
        ...
        ok=ok.and.max_dens(jj)>3.0*d_sink
...
#if USE_FLD==1
!PH commented these terms 22/01/2018
!        if(fld) then
...
!!$        ! Romain master version - To be tested
...
subroutine compute_clump_properties_round2
...
#endif
#if NDUST>0
  use hydro_commons, ONLY: firstindex_ndust, ndust
#endif   
  ...
  !----------------------------------------------------------------------------
  integer::ipart,ilevel,i,peak_nr,global_peak_id,j,ii,jj,ht
...
#if NDUST>0
     do idust=1,ndust
        sum_dust= sum_dust + uold(ind_cell(j),firstindex_ndust+idust)/d
      enddo
#endif 
     call pressure_eos((1.0d0-sum_dust)*d,eint,p)     
     P_Cell(j)=p
...
#if NDUST>0
                    do idust=1,ndust
                       sum_dust= sum_dust + uold(cell_index(j),firstindex_ndust+idust)/d
                    enddo
#endif                    
                    call pressure_eos((1.0d0-sum_dust)*d,eint,P_neigh)                    
