...
#endif
#ifndef NDUST
  integer,parameter::ndust = 0       ! Add a variable to store the number of dust species
  integer,parameter::ndust_pscal=0   ! Add other dust pscals (e.g, s_dust if stepinski growth, T_dust etc..) 
#else
  integer,parameter::ndust = NDUST
#ifndef NDUSTPSCAL
  integer,parameter::ndust_pscal=0   ! Add other dust pscals (e.g, s_dust if stepinski growth, T_dust etc..) 
#else
  integer,parameter::ndust_pscal = NDUSTPSCAL
#endif  
  integer,parameter ::nvardust=ndust+1+(ndust+1)*ndim+ndust_pscal*ndust ! Total number of vars to interpolate 
#endif
...
! Cosmic rays energy groups
...
#if NIONS>0
  integer,parameter::firstindex_nIons=firstindex_pscal ! for ions
  integer,parameter::firstindex_ndust=firstindex_pscal+NIONS ! for dust species
  integer,parameter::firstindex_dustpscal=firstindex_ndust+ndust ! for dust passive scalars 
#else
  integer,parameter::firstindex_ndust=firstindex_pscal ! for dust species
  integer,parameter::firstindex_dustpscal=firstindex_ndust+ndust ! for dust passive scalars 
#endif
...
#if NDUST>0
  real(dp),dimension(1:MAXBOUND,1:ndust)::dust_bound=0.0
#endif  
...
#endif
#if NDUST>0
  real(dp),dimension(1:MAXREGION,1:ndust)::dust_region=0.0
#endif  
  ...
  ! Dust related parameters /!\ TODO : DUST COMMONS
#if NDUST>0  
...
! fin modif nimhd
  ...
  ! Coagulation and self consistent ionization
  integer :: nbincoag            ! Bins for size-distribution
  integer :: ntotchi             ! Number of chi_steps
  double precision :: chi_min,chi_max  ! Min and max chi in the table (excluing 0)
  
  logical :: selfconsistent_nmhd=.false.   ! Activate self consistent ionization computation
  integer :: nvarionis= 3        ! Number of var stored in var_resist
  integer :: nitermax_ionis=100  ! Maximum number of iteration for the ionisation solver
  real(dp):: epsilon_ionis= 1d-5 ! Tolerance for the ionisation solver
  real(dp):: stickeff_el   = 0.5d0  ! Sticking efficiency of electrons
  real(dp):: stickeff_ions = 1.0d0  ! Sticking efficiency of ions
  real(dp):: mu_ions       = 25.0d0 ! Ion mass (in units of mH)
  real(dp):: m_el          = 9.1d-28! Electron mass in grams
  real(dp):: e_el_stat     = 4.8d-10! Electron charge /!\ in statcoulomb here.
  real(dp):: zeta_ionis    = 5d-17  ! Ionisation rate in (s-1)
  logical :: debug_ionis   = .false. ! Debug flag for ionisation
  logical :: icy_grains    = .true.  !Ice mantle
  real(dp):: ice_mantle    = 8.7d-7  !Ice mantle
  real(dp):: psi0          = -3.22   ! Psi lower bound
  real(dp):: thetai        = 107d0   ! Max ratio ni/ne

#if ICOAG>0
  logical :: limit_coag=.true.
#endif
#if THERMAL_IONIZ>0
  character(2):: therm_ion = "Na"     ! Thermally ionized ion
  real(dp) :: mu_ions_s = 29.0d0      ! Thermally-ionized Ion mass (in units of mH)
  real(dp) :: alpt_ioniz=1.4d-15      ! Coefficient for thermal ionization
  real(dp) :: Temp_ioniz=6.0d4        ! Temperature for thermal ionization
  real(dp) :: abund_ioniz=3.1d-9      ! Relative abundance of species
  real(dp) :: kis_ioniz= 6.2d-9       ! Chemical reactions i->s
  real(dp) :: ksi_ioniz= 0d0          ! Chemical reactions s->i
  real(dp) :: sigcoef_a_ioniz= 2.78d-12       ! Recombination coeff
  real(dp) :: sigcoef_b_ioniz= -0.68          ! Recombination exponent
  real(dp) :: qis=1d0                   !Ratio mui/mus
#if ICOAG>0
  integer,parameter::icoag=npscal-4
  integer,parameter::ins_therm=npscal-5 
  integer,parameter::ini_therm=npscal-6 
  integer,parameter::ieps_therm=npscal-7 
  integer,parameter::ipsi_therm=npscal-8  !Index to store psi for faster convergence
  integer,parameter::i_hall=npscal-9
  integer,parameter::i_ad=npscal-10
  integer,parameter::i_ohm=npscal-11
#else
  integer,parameter::ins_therm=npscal-4
  integer,parameter::ini_therm=npscal-5 
  integer,parameter::ieps_therm=npscal-6 
  integer,parameter::ipsi_therm=npscal-7  !Index to store psi for faster convergence
  integer,parameter::i_hall=npscal-8
  integer,parameter::i_ad=npscal-9
  integer,parameter::i_ohm=npscal-10
#endif
#else
!if not thermal ioniz
#if ICOAG>0
  integer,parameter::icoag=npscal-4 
  integer,parameter::ipsi=npscal-5 !Index to store psi for faster convergence
  integer,parameter::i_hall=npscal-6
  integer,parameter::i_ad=npscal-7
  integer,parameter::i_ohm=npscal-8
#else
  integer,parameter::ipsi=npscal-4 !Index to store psi for faster convergence
  integer,parameter::i_hall=npscal-5
  integer,parameter::i_ad=npscal-6
  integer,parameter::i_ohm=npscal-7
#endif
#endif
