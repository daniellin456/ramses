...
!Computes the self consistent values for the resistivities
#if NIMHD==1
  !if(selfconsistent_nmhd) then
                              call timer('Ionisation and resistivities','start')
      call nmhd_resist(ilevel)
      call make_virtual_fine_dp(uold(1,firstindex_pscal+i_ohm),ilevel)
      call make_virtual_fine_dp(uold(1,firstindex_pscal+i_ad),ilevel)
      call make_virtual_fine_dp(uold(1,firstindex_pscal+i_hall),ilevel)
#if THERMAL_IONIZ>0      
      call make_virtual_fine_dp(uold(1,firstindex_pscal+ipsi_therm),ilevel)
      call make_virtual_fine_dp(uold(1,firstindex_pscal+ieps_therm),ilevel)
      call make_virtual_fine_dp(uold(1,firstindex_pscal+ini_therm),ilevel)
      call make_virtual_fine_dp(uold(1,firstindex_pscal+ins_therm),ilevel)
#else
      call make_virtual_fine_dp(uold(1,firstindex_pscal+ipsi),ilevel)
#endif
      if(simple_boundary)call make_boundary_hydro(ilevel)
...
!!$        &  / ((1.d0+1.d0/(2.d0*nsts(ilevel)))**(2*nsts(ilevel)) + (1.d0-1.d0/(2.d0*nsts(ilevel)))**(2*nsts(ilevel)))  ) )
...
#if NDUST>0
#if NDUSTPSCAL>0
     if(dust_growth)call dust_growth_fine(ilevel)
#endif     
#endif 
  ...
  if(rt .and. rt_advect) then
                               call timer('radiative transfer','start')
     call rt_step(ilevel)
     !if (rt_protostar_m1==.true. .and. nsink .gt. 0) call rt_step(ilevel)
  else
  ...
  if((hydro).and.(.not.static_gas))then
#if NDUST>0
                              call timer('dust - ghostzones','start')
     do idim =1,ndim
        do idust=1,ndust
           call make_virtual_fine_dp(v_dust(1,idust,idim),ilevel)
        end do
