...
#if NIMHD==1
#if HALL==1
     if(nhall==1) call computvhall(uin,qin,dx,dy,dz,ngrid,bpred,rppred,vhall)
#endif
#endif
...
#if NIMHD==1
        ! modif nimhd
        emfx(l,i,j,k)=1.d0*emfx(l,i,j,k)+( emfambdiff(l,i,j,k,nxx)+emfohmdiss(l,i,j,k,nxx))*dt/dx
        ! fin modif nimhd
#endif
     end do
  end do
...
#if ICOAG > 0
                 if(n==firstindex_pscal+icoag) then 
                   qRT(l,i,j,k,n,1)=r+dry+drz
                   qRB(l,i,j,k,n,1)=r+dry-drz
                   qLT(l,i,j,k,n,1)=r-dry+drz
                   qLB(l,i,j,k,n,1)=r-dry-drz
                   qRT(l,i,j,k,n,2)=r+drx+drz
                   qRB(l,i,j,k,n,2)=r+drx-drz
                   qLT(l,i,j,k,n,2)=r-drx+drz
                   qLB(l,i,j,k,n,2)=r-drx-drz
                   qRT(l,i,j,k,n,3)=r+drx+dry
                   qRB(l,i,j,k,n,3)=r+drx-dry
                   qLT(l,i,j,k,n,3)=r-drx+dry
                   qLB(l,i,j,k,n,3)=r-drx-dry
                 endif
#endif
              END DO
           END DO
...
! We take the non-updated value for the ionization rate
#if NEXTINCT > 0
               qRT(l,i,j,k,firstindex_extinct+1,1:3)=q(l,i,j,k,firstindex_extinct+1)
               qRB(l,i,j,k,firstindex_extinct+1,1:3)=q(l,i,j,k,firstindex_extinct+1)
               qLT(l,i,j,k,firstindex_extinct+1,1:3)=q(l,i,j,k,firstindex_extinct+1)
               qLB(l,i,j,k,firstindex_extinct+1,1:3)=q(l,i,j,k,firstindex_extinct+1)
#endif
               if(selfconsistent_nmhd) then
                 !r   = q(l,i,j,k,firstindex_pscal+i_hall)            ! Cell centered values
                 !u   = q(l,i,j,k,iu)
                 !v   = q(l,i,j,k,iv)
                 !w   = q(l,i,j,k,iw)
                 !drx = half * dq(l,i,j,k,firstindex_pscal+i_hall,1)   ! TVD slopes
                 !dry = half * dq(l,i,j,k,firstindex_pscal+i_hall,2)
                 !drz = half * dq(l,i,j,k,firstindex_pscal+i_hall,3)
!
                 !qRT(l,i,j,k,firstindex_pscal+i_hall,1) = r + dry + drz
                 !qRB(l,i,j,k,firstindex_pscal+i_hall,1) = r + dry - drz
                 !qLT(l,i,j,k,firstindex_pscal+i_hall,1) = r - dry + drz
                 !qLB(l,i,j,k,firstindex_pscal+i_hall,1) = r - dry - drz
                 !qRT(l,i,j,k,firstindex_pscal+i_hall,2) = r + drx + drz
                 !qRB(l,i,j,k,firstindex_pscal+i_hall,2) = r + drx - drz
                 !qLT(l,i,j,k,firstindex_pscal+i_hall,2) = r - drx + drz
                 !qLB(l,i,j,k,firstindex_pscal+i_hall,2) = r - drx - drz
                 !qRT(l,i,j,k,firstindex_pscal+i_hall,3) = r + drx + dry
                 !qRB(l,i,j,k,firstindex_pscal+i_hall,3) = r + drx - dry
                 !qLT(l,i,j,k,firstindex_pscal+i_hall,3) = r - drx + dry
                 !qLB(l,i,j,k,firstindex_pscal+i_hall,3) = r - drx - dry

! These are just previous values stored for starting point of the newton-raphson scheme, no need to predict them.
#if THERMAL_IONIZ > 0
               qRT(l,i,j,k,firstindex_pscal+ipsi_therm,1:3)=q(l,i,j,k,firstindex_pscal+ipsi_therm)
               qRB(l,i,j,k,firstindex_pscal+ipsi_therm,1:3)=q(l,i,j,k,firstindex_pscal+ipsi_therm)
               qLT(l,i,j,k,firstindex_pscal+ipsi_therm,1:3)=q(l,i,j,k,firstindex_pscal+ipsi_therm)
               qLB(l,i,j,k,firstindex_pscal+ipsi_therm,1:3)=q(l,i,j,k,firstindex_pscal+ipsi_therm)
               qRT(l,i,j,k,firstindex_pscal+ieps_therm,1:3)=q(l,i,j,k,firstindex_pscal+ieps_therm)
               qRB(l,i,j,k,firstindex_pscal+ieps_therm,1:3)=q(l,i,j,k,firstindex_pscal+ieps_therm)
               qLT(l,i,j,k,firstindex_pscal+ieps_therm,1:3)=q(l,i,j,k,firstindex_pscal+ieps_therm)
               qLB(l,i,j,k,firstindex_pscal+ieps_therm,1:3)=q(l,i,j,k,firstindex_pscal+ieps_therm)
               qRT(l,i,j,k,firstindex_pscal+ini_therm,1:3) =q(l,i,j,k,firstindex_pscal+ini_therm)
               qRB(l,i,j,k,firstindex_pscal+ini_therm,1:3) =q(l,i,j,k,firstindex_pscal+ini_therm)
               qLT(l,i,j,k,firstindex_pscal+ini_therm,1:3) =q(l,i,j,k,firstindex_pscal+ini_therm)
               qLB(l,i,j,k,firstindex_pscal+ini_therm,1:3) =q(l,i,j,k,firstindex_pscal+ini_therm)
               qRT(l,i,j,k,firstindex_pscal+ins_therm,1:3) =q(l,i,j,k,firstindex_pscal+ins_therm)
               qRB(l,i,j,k,firstindex_pscal+ins_therm,1:3) =q(l,i,j,k,firstindex_pscal+ins_therm)
               qLT(l,i,j,k,firstindex_pscal+ins_therm,1:3) =q(l,i,j,k,firstindex_pscal+ins_therm)
               qLB(l,i,j,k,firstindex_pscal+ins_therm,1:3) =q(l,i,j,k,firstindex_pscal+ins_therm)
#else
               qRT(l,i,j,k,firstindex_pscal+ipsi,1:3)=q(l,i,j,k,firstindex_pscal+ipsi)
               qRB(l,i,j,k,firstindex_pscal+ipsi,1:3)=q(l,i,j,k,firstindex_pscal+ipsi)
               qLT(l,i,j,k,firstindex_pscal+ipsi,1:3)=q(l,i,j,k,firstindex_pscal+ipsi)
               qLB(l,i,j,k,firstindex_pscal+ipsi,1:3)=q(l,i,j,k,firstindex_pscal+ipsi)
#endif
               end if
             END DO
          END DO
...
SUBROUTINE cmp_mag_flx(qRT,irt1,irt2,jrt1,jrt2,krt1,krt2, &
  ...
  real(dp) :: dx
  real(dp) :: rhoedge,nrjedge,tedge,ionisrate,psiedge,bedge,bedge2
  real(dp) :: dum,dum2,eta
  integer :: ht,niter
...
#endif 
#if THERMAL_IONIZ > 0
  real(dp) :: epsedge,niedge,nsedge
#endif
#if NDUST > 0
  real(dp),dimension(1:ndust) :: n_dust
  real(dp), dimension(1:ndust) :: l_grain,mgrain
#endif
#endif
  ...
  DO k = klo, khi
     DO j = jlo, jhi
        DO i = ilo, ihi
           ...
           ! Compute final fluxes
            DO l = 1, ngrid 
               ...
               if(nhall==1) then
                 ...
                 ! Density
                 rhoedge=0.25*(qLL(l,1)+qLR(l,1)+qRL(l,1)+qRR(l,1))
                 ! Temperature
                 nrjedge=0.25*(qLL(l,2)+qLR(l,2)+qRL(l,2)+qRR(l,2))
                 call temperature_eos(rhoedge,nrjedge/(gamma-1),tedge,ht)
                   ! We take non-updated ionisation rate !!
#if NEXTINCT > 0
                 ionisrate=qRT(l,i,j,k,firstindex_extinct+1,xdim)
#else
                 ionisrate=default_ionisrate
#endif
                  ! Magnetic field
                 bedge=0.25d0*(dsqrt(sum(qLL(l,6:8)**2d0))+dsqrt(sum(qLR(l,6:8)**2d0))+dsqrt(sum(qRL(l,6:8)**2d0))+dsqrt(sum(qRR(l,6:8)**2d0)))

                 if(selfconsistent_nmhd) then

                  ! Coagulation variable
#if ICOAG > 0
                   chiedge=0.25*(qRT(l,i,j,k,firstindex_pscal+icoag,xdim)+qRB(l,i,j,k,firstindex_pscal+icoag,xdim)&
                               &+qLT(l,i,j,k,firstindex_pscal+icoag,xdim)+qLB(l,i,j,k,firstindex_pscal+icoag,xdim))*scale_t
#endif

#if THERMAL_IONIZ > 0
                   ! Thermal ionization variables
                   psiedge=qRT(l,i,j,k,firstindex_pscal+ipsi_therm,xdim)
                   epsedge=qRT(l,i,j,k,firstindex_pscal+ieps_therm,xdim)
                   niedge =qRT(l,i,j,k,firstindex_pscal+ini_therm,xdim)
                   nsedge =qRT(l,i,j,k,firstindex_pscal+ins_therm,xdim)
#else
                   ! Ionization variable
                   psiedge=qRT(l,i,j,k,firstindex_pscal+ipsi,xdim)
#endif
                   ...
                   do idust=1,ndust
                      n_dust(idust) = 0.25d0*(qRT(l,i,j,k,firstindex_ndust+idust)+qRB(l,i,j,k,firstindex_ndust+idust)+qLT(l,i,j,k,firstindex_ndust+idust)+qLB(l,i,j,k,firstindex_ndust+idust))
                   end do
#endif

                   ! Call ionization calculation
                   dum2=1d0
                   niter=0
                   call calc_ioniz(rhoedge*scale_l**3d0,tedge,ionisrate*scale_t,bedge,psiedge,&
#if THERMAL_IONIZ > 0
                                 &epsedge,niedge,nsedge,&
#endif  
#if ICOAG>0
                                 &chiedge,&
#endif
                                 ...
                                 &n_dust,&
#endif
                                 &dum,eta,dum,dx,dum2,niter)
                   eta=eta/bedge

                   qLL (l,firstindex_pscal+i_hall) = eta*bedge
                   qRL (l,firstindex_pscal+i_hall) = eta*bedge
                   qLR (l,firstindex_pscal+i_hall) = eta*bedge
                   qRR (l,firstindex_pscal+i_hall) = eta*bedge
                 else ! If chemistry and not self-consistent
                   bedge2=bedge*bedge
           ...
           ! Compute final fluxes
                 qLL3tmp=qLL (l,3) - eta*vhall(l,i,j,k,xdim,lp1-1)
                 qRL3tmp=qRL (l,3) - eta*vhall(l,i,j,k,xdim,lp1-1)
                 qLR3tmp=qLR (l,3) - eta*vhall(l,i,j,k,xdim,lp1-1)
                 qRR3tmp=qRR (l,3) - eta*vhall(l,i,j,k,xdim,lp1-1)

                 qLL4tmp=qLL (l,4) - eta*vhall(l,i,j,k,xdim,lp2-1)
                 qRL4tmp=qRL (l,4) - eta*vhall(l,i,j,k,xdim,lp2-1)
                 qLR4tmp=qLR (l,4) - eta*vhall(l,i,j,k,xdim,lp2-1)
                 qRR4tmp=qRR (l,4) - eta*vhall(l,i,j,k,xdim,lp2-1)
...
#endif              
#if HALL==1
                  qtmp(firstindex_extinct+1)=vhall(l,i,j,k,4,1)
                  if(selfconsistent_nmhd) qtmp(firstindex_pscal+i_hall)=qLL(l,firstindex_pscal+i_hall)
...
#if HALL==1
                  qtmp(firstindex_extinct+1)=vhall(l,i,j,k,4,1)
                  if(selfconsistent_nmhd) qtmp(firstindex_pscal+i_hall)=qLR(l,firstindex_pscal+i_hall)
...
#if HALL==1
                  qtmp(firstindex_extinct+1)=vhall(l,i,j,k,4,1)
                  if(selfconsistent_nmhd) qtmp(firstindex_pscal+i_hall)=qRL(l,firstindex_pscal+i_hall)
...
#if HALL==1
                  qtmp(firstindex_extinct+1)=vhall(l,i,j,k,4,1)
                  if(selfconsistent_nmhd) qtmp(firstindex_pscal+i_hall)=qRR(l,firstindex_pscal+i_hall)
...
subroutine ctoprim(uin,q,bf,gravin,dt,ngrid)
     ...
     do j = ju1, ju2
        do i = iu1, iu2
           ...
           do l = 1, ngrid
              ...
              sum_dust=0.0d0
              do idust = 1, ndust
                 sum_dust=sum_dust+uin(l,i,j,k,firstindex_ndust+idust)/q(l,i,j,k,1)
              end do
...
#if NEXTINCT > 0
           do l = 1, ngrid
             q(l,i,j,k,firstindex_extinct+1) = ion_rate_fit(uin(l,i,j,k,firstindex_extinct+1),lmh_rate)
           end do
#endif
  ...
  do n = 9+nener, nvar
#if NEXTINCT > 0
    if(n==firstindex_extinct+1) cycle
#endif 
     do k = ku1, ku2
        do j = ju1, ju2
           do i = iu1, iu2
              do l = 1, ngrid
                 ...
                 if(selfconsistent_nmhd) then                  ! Not transforming resistivities
                   if(n==firstindex_pscal+i_ad .or. n==firstindex_pscal+i_hall .or. n==firstindex_pscal+i_ohm) q(l,i,j,k,n)=uin(l,i,j,k,n)
#if THERMAL_IONIZ > 0
                   if(n==firstindex_pscal+ipsi_therm .or. n==firstindex_pscal+ieps_therm .or. n==firstindex_pscal+ini_therm  .or. n==firstindex_pscal+ins_therm) q(l,i,j,k,n)=uin(l,i,j,k,n)
#else
                   if(n==firstindex_pscal+ipsi) q(l,i,j,k,n)=uin(l,i,j,k,n)
#endif
                 end if
              end do
           end do
...
subroutine computdifmag(u,q,ngrid,dx,dy,dz,dt,bemfx,bemfy,bemfz,jemfx,jemfy,jemfz,bmagij,fluxmd,emfohmdiss,fluxohm,jcentersquare)
  ...
  USE amr_parameters
  use hydro_commons
  use hydro_parameters,ONLY:default_ionisrate
...
real(dp)::crossprodx,crossprody,crossprodz, Cv, etax,etay,etaz
     ...
     do j=min(1,ju1+1),max(1,ju2-1)
        do i=min(1,iu1+1),max(1,iu2-1)
           ...
           do l=1,ngrid
              ...
              if(selfconsistent_nmhd) then
                 etax=0.25d0*(u(l,i,j,k,   firstindex_pscal+i_ohm)+u(l,i  ,j-1,k,   firstindex_pscal+i_ohm)+u(l,i,j  ,k-1,   firstindex_pscal+i_ohm)+u(l,i  ,j-1,k-1,   firstindex_pscal+i_ohm))
                 etay=0.25d0*(u(l,i,j,k,   firstindex_pscal+i_ohm)+u(l,i-1,j  ,k,   firstindex_pscal+i_ohm)+u(l,i,j  ,k-1,   firstindex_pscal+i_ohm)+u(l,i-1,j  ,k-1,   firstindex_pscal+i_ohm))
                 etaz=0.25d0*(u(l,i,j,k,   firstindex_pscal+i_ohm)+u(l,i-1,j  ,k,   firstindex_pscal+i_ohm)+u(l,i,j-1,k  ,   firstindex_pscal+i_ohm)+u(l,i-1,j-1,k  ,   firstindex_pscal+i_ohm))
              end if
               if(nmagdiffu .eq.1)then
                 ...
                 bsquarez=bemfz(l,i,j,k,1)**2+bemfz(l,i,j,k,2)**2+bemfz(l,i,j,k,3)**2
                  ...
                  epsz=u(l,i,j,k,nvar)
                  if(selfconsistent_nmhd) then
                     etax=u(l,i,j,k,   firstindex_pscal+i_ohm)
                     etay=u(l,i,j,k,   firstindex_pscal+i_ohm)
                     etaz=u(l,i,j,k,   firstindex_pscal+i_ohm)
                  end if
...
#if NEXTINCT > 0
                ionisrate=q(l,i,j,k,firstindex_extinct+1)
#else
                ionisrate=default_ionisrate 
#endif               
...
!                     endif
                    ...
                    if(selfconsistent_nmhd)  then
                      etaod2 = u(l,i,j,k,firstindex_pscal+i_ohm)
                    else
...
!                    if(nmagdiffu2.eq.1)call temperature_eos(rhocell,u(l,i,j,k,3),tcell,ht)
                    ...
                    if(selfconsistent_nmhd)  then
                      jcentersquare(l,i,j,k) = jcentersquare(l,i,j,k)*u(l,i,j,k,   firstindex_pscal+i_ohm)*dt
                    else
...
! declare local variables
  ...
  real(dp):: betax,betay,betaz,betafx,betafy,betafz
     ...
     do j=min(1,ju1+1),max(1,ju2-1)
        do i=min(1,iu1+1),max(1,iu2-1)
           ...
           do l = 1, ngrid
              ...
              if(selfconsistent_nmhd) then
                 !At corners
                 betax=0.25d0*(u(l,i,j,k,firstindex_pscal+i_ad)+u(l,i,j-1,k,firstindex_pscal+i_ad)+u(l,i,j,k-1,firstindex_pscal+i_ad)+u(l,i,j-1,k-1,firstindex_pscal+i_ad))
                 betay=0.25d0*(u(l,i,j,k,firstindex_pscal+i_ad)+u(l,i-1,j,k,firstindex_pscal+i_ad)+u(l,i,j,k-1,firstindex_pscal+i_ad)+u(l,i-1,j,k-1,firstindex_pscal+i_ad))
                 betaz=0.25d0*(u(l,i,j,k,firstindex_pscal+i_ad)+u(l,i-1,j,k,firstindex_pscal+i_ad)+u(l,i,j-1,k,firstindex_pscal+i_ad)+u(l,i-1,j-1,k,firstindex_pscal+i_ad))
                 !At faces
                 betafx=0.5d0*(u(l,i,j,k,firstindex_pscal+i_ad)+u(l,i-1,j,k,firstindex_pscal+i_ad))
                 betafy=0.5d0*(u(l,i,j,k,firstindex_pscal+i_ad)+u(l,i,j-1,k,firstindex_pscal+i_ad))
                 betafz=0.5d0*(u(l,i,j,k,firstindex_pscal+i_ad)+u(l,i,j,k-1,firstindex_pscal+i_ad))
              endif 
...
!                 bcell=0.5d0*(u(l,i,j,k,2)+u(l,i,j,k-1,2))
             ...
             if(nambipolar2.eq.1)then
              ...
              if(selfconsistent_nmhd) then
                ...
                &u(l,i,j,k,firstindex_pscal+i_ad)*dtlim
              else
                jxbsquare(l,i,j,k)=(jxb(l,i,j,k,1)*jxb(l,i,j,k,1)+jxb(l,i,j,k,2)*jxb(l,i,j,k,2)+jxb(l,i,j,k,3)*jxb(l,i,j,k,3))*&
...
!########################################################################################
#if HALL==1
subroutine computvhall(u,q,dx,dy,dz,ngrid,bpred,rppred,vhall)
  use units_commons
  USE amr_parameters
  use hydro_commons
  use hydro_parameters,ONLY:default_ionisrate
  ...
  real(dp) :: dx,dy,dz
  real(dp) :: rhoedge,nrjedge,tedge,Bedge,bedge2,psiedge
#if ICOAG > 0
  real(dp) :: chiedge
#endif
#if THERMAL_IONIZ > 0
  ...
  real(dp), dimension(1:ndust) :: l_grain,mgrain
#endif
  real(dp) :: eta,ionisrate,dum,dum2
  real(dp) :: eta_hall_chimie
  integer :: niter
      ...
      do l=1,ngrid
        if(nhall==1) then
          do kk=1,3
            ...
            if (kk==1)  vhall(l,i,j,k,1,1:3)=jx(l,i,j,k,1:3)
            if (kk==2)  vhall(l,i,j,k,2,1:3)=jy(l,i,j,k,1:3)
            if (kk==3)  vhall(l,i,j,k,3,1:3)=jz(l,i,j,k,1:3)
...
!!$   (eta_AD_chimie = max(eta_AD_chimie * (1.0d0-tanh(ii/(dble(tchimie))), 1.d-36)
   ...
   eta_AD_chimie=BBcgs**2/(eta_AD_chimie*densionbis(inp,ionisrate)*inp*scale_d*scale_d*clight**2)  ! need B in G, output is gammaad in cgs
...
if(ntestDADM==1) then
  ...
  if(use_x2d==1)then
     ...
     eta_hall_chimie=eta_hall_chimie * clight * clight / (4.0_dp*pi)
...
double precision function densionbis(rhon,zeta)
...
densionbis=coefionis*sqrt(rhoncgs*zeta/1.0d-17)
...
if(ntestDADM.eq.0) then
   ...
   !eetaad=1.4d0/(gammaadbis(rhon)*densionbis(rhon)*rhon/xmneutre )
   ...
   xx=gammaadbis(rhotemp,bsquare,bsquare,temper,ionisrate)*densionbis(rhotemp,ionisrate)*rhotemp
   ...
   if(xx.ne.0.d0) then
      ...
      betaad=1.d39
      if(rhotemp < 1.0d+14.and. .not. selfconsistent_nmhd)then
         write(*,*)'WARNING gammaadbis(rhotemp,bsquare,temper,ionisrate)*densionbis(rhon)*rhon equal 0',gammaadbis(rhotemp,bsquare,bsquare,temper,ionisrate),densionbis(rhotemp,ionisrate),rhotemp
      endif
   endif
...
!    xx=gammaadbis(rhotemp_cell,bsquare,temper)*densionbis(rhocelln)*rhocelln  ! dans la cellule
   ...
   xx=gammaadbis(rhotemp_cell,bsquare,bsquareold,temper,ionisrate)*densionbis(rhotemp_cell,ionisrate)*rhotemp_cell  ! dans la cellule
   ...
   if(xx.ne.0.d0) then
      ...
      betaadbricolo=1.d39
      if(rhotemp < 1.0d+14.and. .not. selfconsistent_nmhd)then
         write(*,*)'WARNING gammaadbis(rhocelln,bsquare,bsquareold,temper,ionisrate)*densionbis(rhocelln)*rhocelln in the cell equals 0',gammaadbis(rhotemp_cell,bsquare,bsquareold,temper,ionisrate),densionbis(rhocelln,ionisrate),rhocelln,bsquare,bsquareold,temper
      endif
   endif
   ...
   xx=gammaadbis(rhotemp,bsquare,bsquareold,temper,ionisrate)*densionbis(rhotemp,ionisrate)*rhotemp  
 ...
 ! a l'interface : cote ou coin selon les cas. A utiliser si l'on est pas dans un cas seuille
   ...
   if(xx.ne.0.d0) then
      ...
      if(rhotemp < 1.0d+14.and. .not. selfconsistent_nmhd)then
         write(*,*)'WARNING gammaadbis(rhon,bsquare,bsquareold,temper,ionisrate)*densionbis(rhon)*rhon at the interface equals 0',gammaadbis(rhotemp,bsquare,bsquareold,temper,ionisrate),densionbis(rhon,ionisrate),rhon
      endif
   endif
