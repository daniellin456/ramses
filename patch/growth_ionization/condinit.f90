...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp) :: psi_iter, psi_0,psi,zeta_adim
  real(dp) :: fpsi,dfdpsi,epsone
  real(dp) :: convergence_ionis,eps_psi,n_i,cross_sectJ_k,nH
...
102     format(i5)
 ...
 ! Radiative transfer
  ...
  if(extinction)q(1:nn,firstindex_extinct+nextinct)=default_ionisrate
#endif
#if ICOAG>0
  ! Coagulation
  q(1:nn,firstindex_pscal+icoag)=0.0D0
#endif
#if NPSCAL>0
  ! passive scalars
  do ivar=1,npscal
...
#if THERMAL_IONIZ>0
       epsone=0.99999
       u(1:nn,firstindex_pscal+ipsi_therm)=psi0
       u(1:nn,firstindex_pscal+ieps_therm)=epsone-1d0/thetai
       u(1:nn,firstindex_pscal+ini_therm)=u(1:nn,1)*1d-7!*scale_l**3d0
       u(1:nn,firstindex_pscal+ins_therm)=u(1:nn,1)*1d-11!*scale_l**3d0
#else
       u(1:nn,firstindex_pscal+ipsi)=psi0
#endif
  end if
