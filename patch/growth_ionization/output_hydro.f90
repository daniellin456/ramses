...
#if ICOAG>0
          if(ivar==icoag) then
             write(ilun,'("variable #",I2,": chi_coag")')firstindex_pscal+3+ivar
             cycle
          end if
#endif
...
#if THERMAL_IONIZ>0             
               case(ipsi_therm)
                 write(ilun,'("variable #",I2,": psi_ioniz")')firstindex_pscal+3+ivar
               case(ieps_therm)
                 write(ilun,'("variable #",I2,": eps_ioniz")')firstindex_pscal+3+ivar
               case(ini_therm)
                 write(ilun,'("variable #",I2,": ni_ioniz")')firstindex_pscal+3+ivar
               case(ins_therm)
                 write(ilun,'("variable #",I2,": ns_ioniz")')firstindex_pscal+3+ivar
#else
! thermal_ioniz
               case(ipsi)
                 write(ilun,'("variable #",I2,": psi_ioniz")')firstindex_pscal+3+ivar
...
! thermal_ioniz
               case(i_ohm)
                 write(ilun,'("variable #",I2,": res_ohm")')firstindex_pscal+3+ivar
               case(i_hall)
                 write(ilun,'("variable #",I2,": res_hall")')firstindex_pscal+3+ivar
               case(i_ad)
                 write(ilun,'("variable #",I2,": res_ad")')firstindex_pscal+3+ivar
               case default
...
!Ndust
     ...
     do ivar=1,npscal-4
        if(firstindex_pscal+ivar .le. firstindex_ndust .or. firstindex_pscal+ivar > firstindex_ndust+ndust ) then
           if(firstindex_pscal+ivar .le. firstindex_ndust) then
              ...
              if(ivar-ndust<10)write(ilun,'("variable #",I2,": passive_scalar_",I1)')firstindex_pscal+3+ivar,ivar-ndust
              if(ivar-ndust.ge.10)write(ilun,'("variable #",I2,": passive_scalar_",I2)')firstindex_pscal+3+ivar,ivar-ndust
           endif
              ...
              if(write_conservative) then
                 do ivar=1,npscal-4 ! Write conservative passive scalars if any
                    do i=1,ncache                      
                       ...
                       if(ivar==i_ad) xdp(i)=uold(ind_grid(i)+iskip,firstindex_pscal+ivar)*sum((0.5*(uold(ind_grid(i)+iskip,6:8)+uold(ind_grid(i)+iskip,nvar+1:nvar+3)))**2d0)
                    end do
                    write(ilun)xdp
                 end do
              else
                 do ivar=1,npscal-4 ! Write passive scalars if any
                    do i=1,ncache
                       !TODO index properly..
!!$#if NDUST>0
!!$                       if(firstindex_pscal+ivar.le.firstindex_dustpscal .or. firstindex_pscal+ivar > firstindex_dustpscal+ndust*ndustpscal) then
!!$                          xdp(i)=uold(ind_grid(i)+iskip,firstindex_pscal+ivar)/max(uold(ind_grid(i)+iskip,1),smallr)
!!$                       else
!!$                          idust_pscal=ivar-firstindex_dustpscal
!!$                          xdp(i)=uold(ind_grid(i)+iskip,firstindex_pscal+ivar)/uold(ind_grid(i)+iskip,(idust_pscal-1)/ndust+idust_pscal)
!!$                       endif
!!$#else
                       xdp(i)=uold(ind_grid(i)+iskip,firstindex_pscal+ivar)/max(uold(ind_grid(i)+iskip,1),smallr)
