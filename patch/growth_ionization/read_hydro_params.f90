subroutine read_hydro_params(nml_ok)
...
#if ICOAG >0
  use hydro_parameters,only:limit_coag
#endif
#if NIMHD==1
  ...
  integer ::idust
#endif  
  real(dp) :: psi,fpsi,epsone,dfdpsi,convergence_ionis
  integer :: niter_ionis
...
!       & ,rsink_max,msink_max,merge_stars &
  ...
  namelist/nonidealmhd_params/nambipolar,gammaAD &
       ...
       & ,coefad, nminitimestep, coefalfven,nmagdiffu2,nambipolar2,nu_sts,coefdtohm,lmh_rate &
       ...
       & ,therm_ion &
#endif
       &,selfconsistent_nmhd, nitermax_ionis,epsilon_ionis,stickeff_el,stickeff_ions,mu_ions,zeta_ionis,debug_ionis,ice_mantle,icy_grains
  ...
  namelist/dust_params/grain_size, grain_dens, K_dust, K_drag,dust_diffusion_test,slope_dust,dust_ratio,mrn, size_min, size_max, mrn_index&
       &,reduce_wdust,reduce_tstop,stokes_max,vdust_max,kwok_correction, vmax_barycenter, f_vmax ,vmax_cs,vmax_dust_lim ,update_eint, smalleps,courant_factor_dust, reduce_dfrac,charged_dust_dynamics
#endif  
  ! fin modif nimhd
...
!                 print*, dtchimie,3.d0/50.d0
        ...
        if(selfconsistent_nmhd) then
          thetai=stickeff_el*sqrt(mu_ions*mH/m_el)
          epsone=0.9999
          convergence_ionis=1d4
          niter_ionis=0
          psi0=-2.5d0
          do while(convergence_ionis>1d-6.and.niter_ionis<10000 .or.  (1.0d0-psi0)/(thetai*exp(psi0))>epsone)
             psi=psi0
             !f(psi)
             fpsi=(1.d0-psi)/(epsone*thetai)*exp(-psi)-1.0d0
             ! df/dpsi
             dfdpsi=-exp(-psi)/(epsone*thetai)-(1.d0-psi)/(epsone*thetai)*exp(-psi)
             !Newton-Raphson step
             psi=psi-fpsi/dfdpsi
             !Convergence check
          ...
          thetai=stickeff_el*sqrt(mu_ions*mH/m_el)
#if THERMAL_IONIZ>0
          select case(therm_ion)  ! Set up the properties of the choosen thermally-ionized ion.
            case("Na")
              alpt_ioniz=1.4d-15
              Temp_ioniz=6.0d4
              abund_ioniz=3.1d-9
              ...
              ksi_ioniz=0d0
              mu_ions_s=23d0
              sigcoef_a_ioniz=2.78d-12
              sigcoef_b_ioniz=-0.68
            case ("K")
              alpt_ioniz=6.5d-15
              Temp_ioniz=5.1d4
              abund_ioniz=2.2d-10
            ...
            case ("H")
              ...
              abund_ioniz=0.5d0
              kis_ioniz=3.7d-14
              ksi_ioniz=6.86d-10+2.0d-9+1.1d-9
              mu_ions_s=1d0
              sigcoef_a_ioniz=3.5d-12
              sigcoef_b_ioniz=-0.75
            case default
              print*, "Wrong choice of therm_ions ! Only Na, K and H available at the moment"
              stop
            end select
            alpt_ioniz=alpt_ioniz!/(scale_l**3d0/scale_t*scale_T2**0.5)
            kis_ioniz=kis_ioniz!/(scale_l**3d0/scale_t)
            ksi_ioniz=ksi_ioniz!/(scale_l**3d0/scale_t)
            sigcoef_a_ioniz=sigcoef_a_ioniz!/(scale_l**3d0/scale_t)
            qis=dsqrt(mu_ions/mu_ions_s)
#endif
        else
          print*, 'must choose an input for abundances or resistivities, or set selfconsistent_nmhd=.true.'
          stop
        endif
     endif
  endif
...
#if ICOAG > 0
  ! Read the coagulation table.
  ! index 1 = chi in cgs
  ! index 1+4(n-1)+1 = size of bin i in cm
  ! index 1+4(n-1)+2 = cross-section of bin i in cm2
  ! index 1+4(n-1)+3 = mass of bin i in g
  ! index 1+4(n-1)+4 = Fractionnal abundance of bin i in cm^-3 per nH. Multiply by nh to have number density, multiply by nh*mass_of_the_bin to have mass density.
  open(43,file="table_coag.dat",form="unformatted",status="old")
  read(43) nbincoag,ntotchi
  allocate(table_coag(ntotchi,1+4*nbincoag))
  read(43) table_coag
  close(43)
  ! Allocate the arrays for the distribution at a given chi.
  allocate(grain_size(nbincoag),grain_cross(nbincoag),grain_mass(nbincoag),grain_abund(nbincoag))
  ! Bin width
  chi_min=table_coag(2,1)
  chi_max=table_coag(ntotchi,1)
  ! Chi to i -> 
  ! If chi<chi_min, i=1
  ! Else i = 2d0 + log(chi/chi_min)/log(chi_max/chi_min)
#endif
  ! End modif coagulation
