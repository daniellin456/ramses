...
#if ICOAG>0
  call add_coag_source_terms(ilevel)
#endif
#if NDUST>0
...
#endif  
              call enerint_eos((1.0d0-sum_dust)*d,0.3d0*Tr_floor,e_prim)              
           ...
           if((nmagdiffu .eq. 1 .or. nambipolar .eq.1 .or. nmagdiffu2 .eq. 1 .or. nambipolar2 .eq.1) .and. nimhdheating_in_eint)then
              ...
              if(nmagdiffu .eq. 1 ) then
                if(selfconsistent_nmhd) then
                  ohm_heating=jsquare*uold(ind_cell(i),firstindex_pscal+i_ohm)
                else
                  ohm_heating=jsquare*etaohmdiss(d,bcell2,Tp_loc,ionisrate) !*dt_imp*vol_loc
                endif
              end if
...
!             take care betaadbricolo should be used
                 if(selfconsistent_nmhd) then
                   ambi_heating = ambi_heating * uold(ind_cell(i),firstindex_pscal+i_ohm)
                 else
...
#endif              
           call pressure_eos((1.0d0-sum_dust)*d,eps,pp_eos)
           do idim=1,ndim
...
!###########################################################
#if ICOAG==1
subroutine add_coag_source_terms(ilevel)
  use amr_commons
  use hydro_commons
  ...
  integer::ilevel
  !--------------------------------------------------------------------------
  ! This routine adds to unew the coagulation source term.
  ! Only the coagulation variable is modified in array unew.
  !--------------------------------------------------------------------------
  integer::i,ind,iskip,ind_cell,ivar
  integer::ht
  real(dp)::d,tempe,u,v,w,e_kin,e_mag,e_prim,e_rad

  if(numbtot(1,ilevel)==0)return
  if(verbose)write(*,111)ilevel

  ! Add coag source term at time t
  do ind=1,twotondim
     iskip=ncoarse+(ind-1)*ngridmax
     do i=1,active(ilevel)%ngrid
        ind_cell=active(ilevel)%igrid(i)+iskip
        ...
        e_mag = 0.0d0
        do ivar=1,3
           e_mag = e_mag + 0.125d0*(unew(ind_cell,5+ivar) &
                &  +unew(ind_cell,nvar+ivar))**2
        end do
        ! Compute total radiative energy
        e_rad=0.0D0
#if NENER>0
        do igroup=1,nener
           e_rad = e_rad + unew(ind_cell,8+igroup)
        end do
#endif
        e_prim=unew(ind_cell,5)-e_kin-e_mag-e_rad
        call temperature_eos(d,e_prim,tempe,ht)
        unew(ind_cell,firstindex_pscal+icoag)=unew(ind_cell,firstindex_pscal+icoag)+&
          & d**1.75d0*tempe**(-0.25d0)*dtnew(ilevel)
        if(limit_coag) unew(ind_cell,firstindex_pscal+icoag)=min(unew(ind_cell,firstindex_pscal+icoag),1.34d18*d/scale_t*(d/1d12)**(1d0/15d0)*(tempe/1d1)**(1d0/15d0))
     end do
  end do

111 format('   Entering add_coag_source_terms for level ',i2)

end subroutine add_coag_source_terms
#endif
! End coag==1
        ...
        do k3=k3min,k3max+1
           do j3=j3min,j3max+1
              do i3=i3min,i3max+1
                 do i=1,ncache
#if THERMAL_IONIZ>0
                    flux(i,i3,j3,k3,firstindex_pscal+ipsi_therm,idim)=0.0d0
                    flux(i,i3,j3,k3,firstindex_pscal+ieps_therm,idim)=0.0d0
                    flux(i,i3,j3,k3,firstindex_pscal+ini_therm,idim)=0.0d0
                    flux(i,i3,j3,k3,firstindex_pscal+ins_therm,idim)=0.0d0
#else
                    flux(i,i3,j3,k3,firstindex_pscal+ipsi,idim)=0.0d0
#endif
                    flux(i,i3,j3,k3,firstindex_pscal+i_ad,idim)=0.0d0
                    flux(i,i3,j3,k3,firstindex_pscal+i_ohm,idim)=0.0d0
                    flux(i,i3,j3,k3,firstindex_pscal+i_hall,idim)=0.0d0
                 end do
              end do
           end do
        end do
