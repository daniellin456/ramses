...
!                       if(isnan(uold(indc2,neulS+1)) .AND. Mdx_cross_loc(ind,ii,mloop,nl) .NE. 0.0) write(*,*) "WARNING LOC: nH2 is NaN and Mdx ne 0 !!!", uold(indc2,1), uold(indc2,neulS+1), Mdx_cross_loc(ind,ii,mloop,nl) 
           ...
           do index_m=1,NdirExt_m
              do index_n=1,NdirExt_n
                 ...
                 extinct = extinct + 1d0/vcolumn_dens(index_m,index_n)!exp(-vcolumn_dens(index_m,index_n)*coef)
              end do
           end do
           ...
           uold(ind_leaf(i),firstindex_extinct+1) = 1d0/coeff_chi*scale_d*scale_l/(scale_d)
...
! Modif ionization rate
! Fit with column density of Padovani et al (2013)
double precision function ion_rate_fit(NH2,lmh_rate)
  use hydro_commons,only:default_ionisrate,dp
  real(dp) :: NH2,NH2_cgs
  character(1) :: lmh_rate
  real(dp) :: xi0_l,xi0_h,a,sig0
  real(dp) :: muu=2.36
  ...
  if(lmh_rate=='l') then
    ...
    a = 0.061
    sig0 = 350
  else if (lmh_rate=='h') then
    xi0_l =  2.635d-14
    xi0_h =  1.045d-17
    a = 1.027
    sig0 = 329
  else if (lmh_rate=='m') then
    xi0_l =  6.8014d-16
    xi0_h =  2.95d-18
    a = 0.463
    sig0 = 357
  else 
    print*, 'wrong lmh_rate !'
    stop
  end if

  if(NH2==0d0) then
    ion_rate_fit = default_ionisrate
  else
    ion_rate_fit = xi0_l*xi0_h/(xi0_h*(NH2/1d20)**a +xi0_l*(exp(muu*1.67d-24*NH2/sig0)-1))
  end if

  ion_rate_fit=max(ion_rate_fit,1d-19)  ! Kunz&Mouschovias 2009, ionisation of radionucleides


end function ion_rate_fit
