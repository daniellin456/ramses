...
subroutine cmpdt(uu,gg,dx,dt,ncell)
...
#if NEXTINCT >0     
     if(isnan(uu(k,firstindex_extinct+1))) uu(k,firstindex_extinct+1)=0d0
     ionisrate(k) = ion_rate_fit(uu(k,firstindex_extinct+1),lmh_rate)
#else     
 ...
 ! Compute maximum dust related speed
...
#if NDUST>0
     do k = 1, ncell
        ctot(k)=ctot(k)+udust(k)/courant_factor_dust
     end do
#endif            
        ...
        do k = 1, ncell
           ...
           if(nhall==1) then
             if(selfconsistent_nmhd) then
               cw1=abs(uu(k,firstindex_pscal+i_hall)/(2.0d0*dx)) ! Whistler wave speed
             else
  ...
  if(nhall.eq.0) then
     ...
     do k = 1,ncell
        if(selfconsistent_nmhd) then
          xx=uu(k,firstindex_pscal+i_hall)
        else
  ...
  if (nmagdiffu.eq.0 .and. nmagdiffu2.eq.0) then
     ...
     do k = 1,ncell
        if(selfconsistent_nmhd) then
          xx=uu(k,firstindex_pscal+i_ohm)
        else
  ...
  if (nambipolar.eq.0 .and. nambipolar2.eq.0) then
     ...
     do k = 1,ncell
        if(selfconsistent_nmhd) then
          xx= B2(k)*uu(k,firstindex_pscal+i_ad)
        else
...
SUBROUTINE find_speed_info(qvar,vel_info)
  ...
  if(nhall==1 .and. dx>0) then
    call temperature_eos(d*(1.0d0-sum_dust),P/(gamma-1),tcell,ht)
    if(selfconsistent_nmhd) then
      eta=qvar(firstindex_pscal+i_hall)*qvar(1)
    else
...
SUBROUTINE find_speed_fast(qvar,vel_info)
...
#if HALL==1
#if NEXTINCT>0
  ionisrate=qvar(firstindex_extinct+1)
#else
  ...
  if(nhall==1 .and. dx>0) then
    call temperature_eos(d*(1.0d0-sum_dust),P/(gamma-1),tcell,ht)
    if(selfconsistent_nmhd) then
      eta=qvar(firstindex_pscal+i_hall)
    else
      eta=abs(eta_hall_chimie(d,tcell,ionisrate,B2))
    endif
