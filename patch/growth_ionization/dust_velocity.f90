subroutine set_vdust(ilevel)
  ...
  ! hydro solver. On entry, hydro variables are gathered from rray uold.
...
subroutine cmpvdust(uin,vout,vdin,dx,dy,dz,dt,ngrid)
  ...
  real(dp) :: d,u,v,w,e_mag,e_kin, sum_dust, enint,thetai_loc,eps_psi,eps_theta
  real(dp) ::pressure, cs,A,B,C,wnorm,vmax, Mach_dv,impl,bcell , tcell
  real(dp) ::  dAy, dAz,dBx,dBz,dCx,dCy,divB,psi
  real(dp),dimension(1:ndim) :: fpress,fmag,grad_P,curl_BcrossB
  real(dp),dimension(1:ndust)  :: t_stop,f_cyclotron_dust
  real(dp)  ::pi,tstop_tot,t_stop_floor, epsilon_0
  real(dp), dimension(1:ndust) ::d_grain,l_grain
  real(dp),dimension(1:ndim):: ew
  real(dp),dimension(1:ndust):: dustMRN,tau_k,alpha_k
  real(dp),dimension(1:ndust,1:ndim):: wdust_Hydro
  
  real(dp):: betaadbricolo,etaohmdissbricolo, betaad,etaohmdiss,eta_hall_chimie, etahall,ionisrate,e_el,clight,B_gauss
  ...
  thetai_loc=stickeff_el*sqrt(mu_ions*mH/m_el)
           ...
           do l = 1, ngrid
                 ...
                 if(charged_dust_dynamics) then
                    ...
                    if(nmagdiffu .eq.1 .or.nmagdiffu2 .eq.1) then
                       etaohmdiss=uin(l,i,j,k,firstindex_pscal+i_ohm)! etaohmdissbricolo(d,bcell,tcell,dt,dx,ionisrate)
                       Enimhd=Enimhd+etaohmdiss*curl_B
                    endif
                    !Ambipolar resistivity
                    if(nambipolar .eq. 1.or.nambipolar2 .eq. 1) then
                       betaad = uin(l,i,j,k,firstindex_pscal+i_ad)
                    ...
                    ! We compute f_cyclotron_dust in cgs to avoid making units errors and rescale it
                    B_gauss= 4.0*pi*sqrt(Bcell*scale_d*scale_l**2/scale_t**2.)
                    !if(spitzer_charging) then
                    !   do idust=1,ndust
                    !      Z_dust(idust)=-kB*tcell*l_grain(idust)*scale_l/(e_el**2.*clight**2.)
                    !   end do
                    !end if
                    psi=uin(l,i,j,k,firstindex_pscal+ipsi)
                    tau_k= (l_grain*scale_l)*kB*tcell/e_el_stat**2. ! l_grain in cgs to keep tau_k dimensionless
                    alpha_k=sqrt(8.0d0/(pi*tau_k))
                    eps_psi=(1.0d0-psi)/thetai_loc*exp(-psi)
                    eps_theta=eps_psi*thetai_loc
                    z_dust(:)=psi*tau_k(:)+(1.0d0-eps_theta**2.0)/(1.0d0+eps_theta*alpha_k(:)+eps_theta**2.0)
                    do idust=1,ndust
                       f_cyclotron_dust(idust)=(Z_dust(idust)*e_el*B_gauss/(4./3.*pi*d_grain(idust)*l_grain(idust)**3.*scale_m))*scale_t
#if NDUSTPSCAL>0
                       if(stepinski) f_cyclotron_dust(idust)=(Z_dust(idust)*e_el*B_gauss/(4./3.*pi*d_grain(idust)*(uin(l,i,j,k,firstindex_dustpscal+idust)/uin(l,i,j,k,firstindex_ndust+idust))**3.*scale_m))*scale_t
#endif                      
                    end do
                    gamma_dust=t_stop*f_cyclotron_dust
