...
!     end do
     ...
     if(isothermal) then
        do i=1,nleaf
           d=MAX(uold(ind_leaf(i),1),smallr)
           sum_dust=0.0d0
           do idust=1,ndust
              sum_dust=sum_dust+uold(ind_leaf(i),firstindex_ndust+idust)/d
           end do
           T2min(i)=(1.0d0-sum_dust)*d*cs**2./(gamma-1.0d0)
