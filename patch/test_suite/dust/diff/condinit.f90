...
subroutine condinit(x,u,dx,nn)
  ...
  real(dp)::xn,x0,sum_dust, t_stop, sound_speed,rho_0, epsilon_0, r_c
      ...
      do i=1, nn
         ...
         sum_dust =0.0d0
         do idust = 1, Ndust
              if (1.0_dp .gt. (xn-x0)*(xn-x0)/r_c/r_c ) then
                 q(i,firstindex_ndust+idust) = epsilon_0*(1.0_dp - (xn-x0)*(xn-x0)/r_c/r_c)
                 sum_dust= sum_dust+ q(i,firstindex_ndust+idust)

              else
                 q(i,firstindex_ndust+idust) = 0.0_dp
                 sum_dust= sum_dust+ q(i,firstindex_ndust+idust)
              end if
         end do     
         q(i,5)= sound_speed*sound_speed*rho_0*(1.0_dp-sum_dust)
      end do
