subroutine read_hydro_params(nml_ok)
  ...
  ! Local variables
  !--------------------------------------------------
  integer::i,j,irad,nboundary_true=0,ht
  ...
  namelist/hydro_params/gamma,courant_factor,smallr,smallc,smallcr &
       & ,niter_riemann,slope_type,slope_mag_type,switch_solv,switch_solv_dens, dtdiff_params,dt_control &
#if NENER>0
       & ,gamma_rad &
#endif
...
!       & ,rsink_max,msink_max,merge_stars &
  ...
  ! fin modif nimhd
#if NDUST>0
  namelist/dust_params/grain_size, grain_dens, K_dust, K_drag,slope_dust,dust_ratio,mrn, size_min, size_max, mrn_index,dust_diffusion_test&
       &,reduce_wdust,vdust_max,kwok_correction, vmax_barycenter, f_vmax ,vmax_cs,vmax_dust_lim ,update_eint, smalleps,courant_factor_dust
#endif
  ! Read namelist file
  rewind(1)
...
109 continue
  ...
  do i=1,nboundary
     ...
     call temperature_eos(d_bound(i)*(1.0d0-sum_dust),P_bound(i)/(gamma-1.0d0),T_bound(i),ht)
     do j=1,ngrp
