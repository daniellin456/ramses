...
!PH 12/01/2020 commented this, too many neg dens with SN and 200 pc box
...
!                             column_dens(i,m,n) = column_dens(i,m,n) + dx_cross_ext*uold(cell_ind2,1)   
                             ...
                             H2column_dens(i,mloop,nl) = H2column_dens(i,mloop,nl) + dx_cross_ext*uold(cell_ind2,neulS+1)*weight
...
!  real(dp), dimension(1:NdirExt_m,1:NdirExt_n),intent(inout) :: vcolumn_dens
  ...
  real(dp)            :: TTold, ref,dRefdT, eps, vardt,varrel, dTemp,ref2
  real(dp)            :: rhoutot2
  real(dp)            :: scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2
  ...
  if (NN .le. smallr) then 
!PH 12/01/2020 changed this because too many neg dens when SN and 200 pc box
!     if( NN .le. 0)  write(*,*) 'prob dens',NN
     NN = smallr  !max(NN,smallr)
  endif
...
!======================================================================
!PH: impose smallr to avoid negative density and recalculate momentum and total energy to impose velocity 
!and tehrmal energy are unchanged
!(19/01/2020)
     ...
     !retrieve kinetic energy from total energy
     do i=1,nleaf
        uold(ind_leaf(i),neulP) = uold(ind_leaf(i),neulP) - ekin(i)
     end do


     !conserve velocity (not momentum)
     do idim=1,3
        do i=1,nleaf
           uold(ind_leaf(i),idim+1) = uold(ind_leaf(i),idim+1) / uold(ind_leaf(i),1) * max(uold(ind_leaf(i),1),smallr)
        end do
     end do
...
!        if(isnan(T2(i)) .OR. T2(i) .LT. 0) stop
        !PH 12/01/2020 changes this. While doing supernovae T<0 appears
        if(isnan(T2(i))) stop
        !PH 12/01/2020 added this. Inspired from what is done in calc_temp
        if (T2(i) .lt.0) then
!            write(*,*) 'prob Temp',TT, NN
            !         write(*,*) 'repair assuming isobariticity'
            T2(i) = min(4000./max(uold(ind_leaf(i),1),smallr),10000.)  !2.*4000. / NN
        endif
