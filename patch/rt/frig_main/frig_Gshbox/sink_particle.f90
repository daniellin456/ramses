...
!     if(msink(isink) .gt. 100.*msink_all(isink)) xsink(isink,1:ndim)=xsink(isink,1:ndim)+xsink_all(isink,1:ndim)/msink(isink)
...
subroutine update_sink(ilevel)
  ...
  do isink=1,nsink
     ...
     !CC 03/17 : shearing box of the sinks                                                                                                        

     if(xsink(isink,2)<0d0) then
        xsink(isink,2)=mod(xsink(isink,2),boxlen)+boxlen
        xsink(isink,1)=mod(xsink(isink,1)+Vshear*t,boxlen)
        if(xsink(isink,1)<0d0) xsink(isink,1)=xsink(isink,1)+boxlen
        vsink(isink,1)=vsink(isink,1)+Vshear
     end if

     if(xsink(isink,2)>boxlen) then
        xsink(isink,2)=mod(xsink(isink,2),boxlen)
        xsink(isink,1)=mod(xsink(isink,1)-Vshear*t,boxlen)
        if(xsink(isink,1)<0d0) xsink(isink,1)=xsink(isink,1)+boxlen
        vsink(isink,1)=vsink(isink,1)-Vshear
     end if
