...
!-------------------------------------------------------------------------
  ...
  use SED_module
  use UV_module
  use pm_commons !for hybrid RT, needs sink quantities
  implicit none
...
!-------------------------------------------------------------------------
  namelist/rt_params/rt_star, rt_esc_frac, rt_flux_scheme, rt_smooth     &
       ...
       & ,rt_isoPress, rt_isIRtrap, rt_movie_vars, rt_err_n              &
...
101 continue                                   ! No harm if no rt namelist
  ...
  if(rt_err_grad_n .gt. 0. .or. rt_err_grad_xHII .gt. 0.                 &
       .or. rt_err_grad_xHI .gt. 0. .or. rt_err_n .gt. 0.) rt_refine=.true.
...
!-------------------------------------------------------------------------
  ...
  use rt_cooling_module
  use SED_module
  !use radiation_parameters !raph for kappaAbs=kplanck(Tgas,Tstar) and kappaSc, for pascucci's test but for collapse Tstar is from the sink
  use hydro_parameters, only: ngrp
  use pm_commons
  use cloud_module,only:rt_protostar_m1
  implicit none
  integer::i, igrp !igrp for kappaAbs=kplanck(Tgas,Tstar) and kappaSc
!  real(dp)::planck_ana, planck_ana_scat !raph for kappaAbs=kplanck(Tgas,Tstar)
  real(dp)::Tstar
!-------------------------------------------------------------------------
...
!    end function planck_ana_scat
  ...
  if (nsink .gt. 0 .and. rt_protostar_m1 .and. ngrp==1) then
     Tstar = Teff_sink(1)
     !kappaAbs = planck_ana(1.0d0, 1000.d0, Tstar ,1) !M1 abs opacity at Tstar, careful with Tgas
...
!     if(myid==1 .and. isoscat) write(*,*) "M1 scattering is Planck's mean opacity at the star temperature"
     ...
     if(myid==1) write(*,*) "ngrp>1 or no sink so M1 absorption and scattering opacities are taken from the namelist"
  endif  
...
!  use pm_commons
  ...
  real(dp)::radiation_source,lum_group!,rstar_adim,Tstar
...
#if NDIM>1
            !  lum_group = radiation_source(Tstar,1)/(scale_d*scale_v**2)*(pi*rstar_adim**2*clight/scale_v)/(2.d0*rstar_adim)
              !to get the same flux at rstar_adim as in 3D
#endif
#if NDIM>2
