...
if(ndim>2) then
  ...
  write(ilun,'("variable #",I2,": radiative_acceleration_x")')ivar
  ivar=firstindex_pscal+3+npscal+3
  write(ilun,'("variable #",I2,": radiative_acceleration_y")')ivar
  ivar=firstindex_pscal+3+npscal+4
  write(ilun,'("variable #",I2,": radiative_acceleration_z")')ivar
...
subroutine backup_hydro(filename)
  use amr_commons
  use hydro_commons
  use radiation_parameters,only:frad
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
     ...
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           ...
           do ind=1,twotondim
              ...
              ! Write radiative acceleration
              do ivar=1,3
                 do i=1,ncache
                    xdp(i)=frad(ind_grid(i)+iskip,ivar)
                 end do
                 write(ilun)xdp
              end do
