subroutine init_hydro
  ...
  if(fld)then
     ...
     !allocate(pheating(1:ncell))
     rad_flux=0.0d0; urad=0.0d0; frad=0.0d0! ; pheating=0.0d0
  endif
...
!      if(nvar2.ne.nvar+4)then
     if(.not.(neq_chem.or.rt) .and. nvar2.ne.nvar+7)then !4->7 raph
        write(*,*)'File hydro.tmp is not compatible'
        write(*,*)'Found   =',nvar2
        write(*,*)'Expected=',nvar+7 !4->7 raph
        call clean_stop
     end if
#ifdef RT
     if((neq_chem.or.rt).and.nvar2.lt.nvar+7+NGroups*(ndim+1))then ! OK to add ionization fraction vars !4->7 raph 
        ...
        if(myid==1) write(*,*)'Expected=',nvar+7+NGroups*(ndim+1)!4->7 raph
        if(myid==1) write(*,*)'..so only reading first ',nvar2, &
                  'variables and setting the rest to zero'
     end if
     if((neq_chem.or.rt).and.nvar2.gt.nvar+7+NGroups*(ndim+1))then ! Not OK to drop variables!4->7 raph
        if(myid==1) write(*,*)'File hydro.tmp is not compatible'
        ...
        do ibound=1,nboundary+ncpu
           ...
           if(ncache>0)then
              ...
              do ind=1,twotondim
                 ...
                 if(write_conservative) then
#ifdef RT
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7-NGroups*(ndim+1))-4 ! Read conservative passive scalars if any!le premier -4->-7 raph
#else
                    !do ivar=1,npscal-4 ! Read conservative passive scalars if any
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7)-4 ! Read conservative passive scalars if any !le premier -4->-7 raph
#endif
                       read(ilun)xx
                       do i=1,ncache
                          !uold(ind_grid(i)+iskip,firstindex_pscal+ivar)=xx(i)
                          uold(ind_grid(i)+iskip,ivar)=xx(i)!*max(uold(ind_grid(i)+iskip,1),smallr) !not conservative with *max() ?
                       end do
                    end do
                 else
#ifdef RT
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7-NGroups*(ndim+1))-4 ! Read passive scalars if any!le premier -4->-7 raph
#else
                    !do ivar=1,npscal-4 ! Read passive scalars if any
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7)-4 ! Read passive scalars if any!le premier -4->-7 raph
#endif
                       read(ilun)xx
                       do i=1,ncache
...
#ifdef RT
                 do ivar=min(nvar,nvar2-7)-3,min(nvar,nvar2-7-NGroups*(ndim+1))-1 ! Read current !-0->-7 et -4->-7 raph
#else
                 !do ivar=npscal-3,npscal-1 ! Read current
                 do ivar=min(nvar,nvar2-7)-3,min(nvar,nvar2-7)-1 ! Read current !-0->-7 et -4->-7 raph
#endif
                    read(ilun)xx
                    do i=1,ncache
                       !uold(ind_grid(i)+iskip,firstindex_pscal+ivar)=xx(i)
                 ...
                 if(write_conservative) then
#ifdef RT
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7-NGroups*(ndim+1))-1 ! Read conservative passive scalars if any !-4->-7 raph
#else
                    !do ivar=1,npscal-1 ! Read conservative passive scalars if any
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7)-1 ! Read conservative passive scalars if any !-4->-7 raph
#endif
                       read(ilun)xx
                       do i=1,ncache
                          !uold(ind_grid(i)+iskip,firstindex_pscal+ivar)=xx(i)
...
#ifdef RT
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7-NGroups*(ndim+1))-1 ! Read passive scalars if any!-4->-7 raph
#else
                    !do ivar=1,npscal-1 ! Read passive scalars if any
                    do ivar=firstindex_pscal+1,min(nvar,nvar2-7)-1 ! Read passive scalars if any!-4->-7 raph
#endif
                       read(ilun)xx
                       do i=1,ncache
                          !uold(ind_grid(i)+iskip,firstindex_pscal+ivar)=xx(i)*max(uold(ind_grid(i)+iskip,1),smallr)
                 ...
                 do ivar=1,3
                    read(ilun)xx
                    do i=1,ncache
                       frad(ind_grid(i)+iskip, idim)=xx(i)
                    enddo
                 enddo
                 !raph
