subroutine diffusion_cg (ilevel,Nsub)
  ...
  use cloud_module, only: rt_feedback !raph, hybrid RT
  use hydro_commons
  use rt_hydro_commons !raph, hybrid RT
  use radiation_parameters
  use rt_cooling_module !raph, hybrid RT
  ...
  real(dp)::max_loc
  integer ::im1 !raph, hybrid RT, index of M1 group. Not tested for >1 grp
...
#ifdef RT
     if(rt_protostar_m1 .and. rt_advect .and. nsink >0 .and. Teff_sink(1) > 0.0d0)then !raph, heating term by M1 photons absorbed
        do im1=1,ngroups !!! Loop over M1 photon groups
           ...
           !pheating(liste_ind(i)) = protostellar_heating      
           !rtuold(liste_ind(i),iGroups(im1))*scale_Np*group_egy(im1)*ev_to_erg*planck_ana(1.0d0,Told,Teff_sink(1),1)*rho*scale_d*rt_c_cgs*z_ave / (scale_d*scale_v**2)  
           end do
     end if
#endif
...
subroutine compute_residual_in_cell(i,vol_loc,residual,mat_residual)
  ...
  use radiation_parameters
  use rt_cooling_module !raph, hybrid RT
  use cloud_module, only : rt_protostar_m1,rt_feedback !raph, hybrid RT
  use units_commons
  use const
#ifdef RT 
  use pm_commons !raph, hybrid RT, needs sink quantities
...
#ifdef RT
  call rt_units(scale_Np,scale_Fp) !raph, hybrid RT
#endif
  ...
  do igroup=1,ngrp
     ...
     protostellar_heating = 0.0d0
#ifdef RT
     if(rt_protostar_m1 .and. rt_advect .and. nsink >0 .and. Teff_sink(1) > 0.0d0)then !raph, heating by M1 photons
        do im1=1,ngroups
     ...
     residual(igroup) = uold(i,firstindex_er+igroup)*vol_loc  &
          ...
          & + vol_loc*wdtB(igroup)*deriv(igroup)/scale_E0*protostellar_heating*dt_imp*scale_t/(cv+lhs) & !M1 heating into CvT
