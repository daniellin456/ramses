...
subroutine radiative_feedback_sink(ilevel)
  ...
  use amr_commons
  use hydro_commons
  use cloud_module,only: rt_protostar_m1,lum_injection
     ...
     do ind=1,twotondim  
        ...
        do i=1,ngrid
           if(ok(i))then
              ...
              in_sink(ind_cell(i))=.false. ! raph 30/04/20 all sinks
              ...
              do isink=1,nsink
                 ...
                 if(drr.lt.rmax2)then
                    !in_sink(ind_cell(i))=.true. ! raph 30/04/20 all sinks
                    q = rr/h_loc
                    !              
                    ...
                    if(Tstar .gt. 0)then
                       if(.not. rt_protostar_m1 .or. isink .gt. 1)then                       
                          if(stellar_photon)then
                             ...
                             uold(ind_cell(i),5     )=uold(ind_cell(i),5     ) + Lum_sink(isink)*weight*dtnew(ilevel)!/(8.0d0*(rmax/4.0d0)**3)!((4.0d0*pi*rmax**3)/3.0d0)
                             uold(ind_cell(i),8+igrp)=uold(ind_cell(i),8+igrp) + Lum_sink(isink)*weight*dtnew(ilevel)!/(8.0d0*(rmax/4.0d0)**3)!((4.0d0*pi*rmax**3)/3.0d0)
                          else
                             do igrp=1,ngrp 
                                Lum_group = radiation_source(Tstar,igrp)/(scale_d*scale_v**2)*(pi*rsink_star(isink)**2*clight/scale_v)!/(8.0d0*(rmax/4.0d0)**3)!((4.0d0*pi*rmax**3)/3.0d0)
                       ...
                       if(rt_protostar_m1 .and. isink ==1)then
                          ...
                          Lum_group = aR*(Tstar**4)/(scale_d*scale_v**2)*(pi*rsink_star(isink)**2*clight/scale_v)!/(8.0d0*(rmax/4.0d0)**3)!/((4.0d0*pi*rmax**3)/3.0d0) !*0.1d0 or *5 to mimic change in PMS track
                          !division added by raph, needed if weight=1 or kernel=1
                          rtunew(ind_cell(i),1)=rtunew(ind_cell(i),1) + Lum_group*weight*dtnew(ilevel)/((group_egy(1)*ev_to_erg)/scale_d/scale_v**2)                          
                          if(sinks_opt_thin==.true.) in_sink(ind_cell(i))=.true. ! raph 29/04/2020 
                       endif
#endif
                    end if
