...
#ifdef RT
     if((neq_chem.or.rt).and.nvar2.lt.nvar+7)then ! OK to add ionization fraction vars
        ...
        if(myid==1) write(*,*)'Expected=',nvar+7
        if(myid==1) write(*,*)'..so only reading first ',nvar2, &
                  'variables and setting the rest to zero'
     end if
     if((neq_chem.or.rt).and.nvar2.gt.nvar+7)then ! Not OK to drop variables
        if(myid==1) write(*,*)'File hydro.tmp is not compatible'
