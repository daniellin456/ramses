subroutine file_descriptor_hydro(filename)
  ...
  use rt_parameters
  use cloud_module, only: rt_protostar_m1,rt_protostar_fld
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  ...
  integer::ivar,ilun,firstindex_frad!,firstindex_kabs
  !integer::firstindex_ph !pheating
...
#ifdef RT
  write(ilun,'("nvar        =",I11)')nvar+7+NGroups*(ndim+1) !+4=>+7 pour frad raph
#else
  write(ilun,'("nvar        =",I11)')nvar+7 !+4=>+7 pour frad raph
#endif  
  ivar=1
...
if(ndim>2) then
  ...
  if (rt_protostar_m1==.true. .and. rt_protostar_fld==.false.) then
     firstindex_frad = nvar + 8
  elseif (rt_protostar_m1==.false. .and. rt_protostar_fld==.true.) then
     firstindex_frad = nvar + 4
  endif

  ivar = 1
  write(ilun,'("variable #",I2,": radiative_acceleration_fld_x")')firstindex_frad+ivar!,ivar should not be here, take example on velocity
  ivar = 2
  write(ilun,'("variable #",I2,": radiative_acceleration_fld_y")')firstindex_frad+ivar
  ivar = 3
  write(ilun,'("variable #",I2,": radiative_acceleration_fld_z")')firstindex_frad+ivar
  !raph pour force radiative

#else
  ! Radiative acceleration 
  ivar=firstindex_pscal+3+npscal+2
  write(ilun,'("variable #",I2,": radiative_acceleration_fld_x")')ivar
  ivar=firstindex_pscal+3+npscal+3
  write(ilun,'("variable #",I2,": radiative_acceleration_fld_y")')ivar
  ivar=firstindex_pscal+3+npscal+4
  write(ilun,'("variable #",I2,": radiative_acceleration_fld_z")')ivar
  !raph pour force radiative
#endif
...
!  use radiation_parameters,only:pheating !raph pour protostellar heating
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
...
!   endif
#ifdef RT
  write(ilun)nvar+7+NGroups*(ndim+1) !+4=>+7 raph pour frad
#else
  write(ilun)nvar+7 !+4=>+7 raph pour frad
#endif
  write(ilun)ndim
...
#ifdef RT
              do ivar=1,nGroups
                 !if(write_conservative) then! Store photon density in conservative way
                 !   do i=1,ncache
                 !      xdp(i)=rtuold(ind_grid(i)+iskip,iGroups(ivar))
                 !   end do
                 !   write(ilun)xdp
                 !else    ! Store photon density in flux units
                    do i=1,ncache
