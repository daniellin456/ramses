...
SUBROUTINE rt_solve_cooling(T2, xion, Np, Fp, p_gas, dNpdt, dFpdt        &
                           ,nH, c_switch, Zsolar, dt, a_exp, nCell,insink)!!!raph 28/04/2020 for sink opt thin
...
!-------------------------------------------------------------------------
  ...
  real(dp)::dt, a_exp
  integer::ncell
  logical,dimension(1:nvector)::insink!--------------------------------------------------------
...
!  if(myid==1) print*, "rt solve cooling begins, p gas= , dp_gas= ", p_gas, dp_gas
...
contains
  ...
  !-----------------------------------------------------------------------
    ...
    use const
    use cloud_module,only:rt_protostar_m1 !raph, hybrid RT
    use pm_commons !raph, hybrid RT, needs sink quantities
    implicit none
    ...
    dFp(:,:)=Fp(:,:,icell) ; dp_gas(:)=p_gas(:,icell) !dp=0 : raph test dp=0 instead of p puis dp=dp-p
    ...
    rho = nH(icell) / X * mH
     !!! raph, if hybrid RT : M1 abs opacity is Planck's mean at Tstar
    if (nsink .gt. 0 .and. Teff_sink(1) .gt. 0.0d0 .and. rt_protostar_m1 .and. ngrp==1) then
    ...
    if(rt .and. nGroups .gt. 0) then
       ...
       if(is_kIR_T) then                           ! k_IR depends on T_rad
          ...
          kAbs_loc(iIR) = kappaAbs(iIR) * (TK/10d0)**2 * exp(-TR/2d3)
          kSc_loc(iIR)  = kappaSc(iIR)  * (TK/10d0)**2 * exp(-TR/2d3)
...
!!$          kSc_loc(iIR)  = kappaSc(iIR)  * (TR/10d0)**2 * exp(-TR/1d3)
       ...
       dustAbs(:)  = kAbs_loc(:) *rho*z_ave*rt_c_cgs!Zsolar(icell)*rt_c_cgs !Hack for Z=1 otherwise no absorption
       dustSc(iIR) = kSc_loc(iIR)*rho*z_ave*rt_c_cgs!Zsolar(icell)*rt_c_cgs
      ! if(myid==1) print*, "dustabs, kAbsloc, rho, rt_c, TK= ", dustAbs(:), kAbs_loc(:), rho, rt_c_cgs, TK
    ...
    if(rt .and. rt_advect) then
       ...
       do igroup=1,nGroups ! -------Momentum transfer from photons to gas:
          ...
          !if(myid==1) print*, "dt, phabs, phSc, grpegyerg,1/c= ", ddt(icell),phAbs(igroup),phSc(igroup), group_egy_erg(igroup),one_over_c_cgs
          ...
          !   if(myid==1) print*, "dmom,  momfact, dNp, fluxmag, dFp",dmom(idim), mom_fact, dNp(igroup), fluxMag, dFp(idim,igroup)
          end do
       end do
       dp_gas = dp_gas + dmom * rt_pressBoost        ! update gas momentum
      ! if(myid==1) print*, "update gas momentum, dp gas=, dmom*rtpressboost= ", dp_gas,  dmom * rt_pressBoost
    ...
    dp_gas(:)= dp_gas(:)-p_gas(:,icell) !dp=dp : raph test dp=0 instead of p puis dp=dp-p
!    if(myid==1 .and. dp_gas(1) .gt. 0.0d0) print*, "0<dp gas=dp-p= , p_gas, T, rho= ", dp_gas(:), p_gas(:,icell), T2(icell), rho
...
!-------------------------------------------------------------------------
  ...
  logical,dimension(1:nvector)::insink!!!raph 28/04/2020 for sink opt thin
!-------------------------------------------------------------------------
  aexp = astart
