...
! Cosmic rays energy groups
  ...
  integer :: ndust_max_mrn= ndust                        ! To truncate the mrn at n<ndust
...
! fin modif nimhd
  ...
  !seed_pscal,seed_high_T,i_seed,T_seed,evaporate_passive,T_evaporate
  ! Passive scalars related variables
  logical :: seed_pscal = .false.
  integer :: nseeds = 0
  !integer,dimension(1:NPSCAL) :: seed_type = -1
  !real(dp),allocatable,dimension(1:NPSCAL,1:3) :: x_seed = 0.
  !real(dp),allocatable,dimension(1:NPSCAL,1:3) :: ax_seed = 0.
  !real(dp),allocatable,dimension(1:NPSCAL) :: r_seed = 0.
  integer,allocatable,dimension(:) :: seed_type
  real(dp),allocatable,dimension(:,:) :: x_seed
  real(dp),allocatable,dimension(:,:) :: ax_seed
  real(dp),allocatable,dimension(:) :: r_seed
  logical :: seed_high_T = .false.
  integer,dimension(1:NPSCAL) :: i_seed = 0
  real(dp),dimension(1:NPSCAL):: T_seed = 0.
  logical :: evaporate_pscal = .false.
  real(dp),dimension(1:NPSCAL):: T_evaporate = 100000.
