...
#endif
  use hydro_parameters, ONLY: disk_setup
...
real(kind=8),dimension(n_zint)::comm_buffin,comm_buffout
  ...
  !-------------------------------------------------------
     ...
     ! End loop over grids
     ! Call initial condition routine for B field
     if (disk_setup .ne. 111) then
       call mag_compute(ilevel)
     endif
#ifndef WITHOUTMPI
if ((mod(disk_setup,10.d0) == 1) .and. (ilevel .eq. nlevelmax) .and. first_lmax .and. (.not. disk_readTable) ) then
  if (disk_TableLevel .eq. -1) disk_TableLevel = nlevelmax
  if ( (disk_setup .ge. 201) .and. (disk_setup .lt. 10001) ) then
    if (disk_setup .ge. 1001) then
      comm_buffin = P0_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
        &MPI_COMM_WORLD,info)
      P0_table = comm_buffout
      comm_buffin = P1_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
        &MPI_COMM_WORLD,info)
      P1_table = comm_buffout
      comm_buffin = P2_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
        &MPI_COMM_WORLD,info)
      P2_table = comm_buffout
      comm_buffin = P3_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
        &MPI_COMM_WORLD,info)
      P3_table = comm_buffout
      if (myid .eq. 1) then
        open(4242,file="Boundary_table.txt",status='new')
        do i=1,2**(disk_TableLevel-1)
          write(4242,*) P0_table(i), P1_table(i), P2_table(i), P3_table(i)
        enddo
        close(4242)
      endif
      first_lmax = .false.
    else
      comm_buffin = vrot_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
           &MPI_COMM_WORLD,info)
      vrot_table = comm_buffout
      comm_buffin = p_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
           &MPI_COMM_WORLD,info)
      p_table = comm_buffout
      comm_buffin = d_table
      comm_buffout = 0.d0
      call MPI_ALLREDUCE(comm_buffin,comm_buffout,n_zint,MPI_DOUBLE_PRECISION,MPI_SUM,&
           &MPI_COMM_WORLD,info)
      d_table = comm_buffout
      if (myid .eq. 1) then
        open(4242,file="Boundary_table.txt",status='new')
        do i=1,2**(disk_TableLevel-1)
          write(4242,*) vrot_table(i), p_table(i), d_table(i)
        enddo
        close(4242)
      endif
      first_lmax = .false.
...
else if ((mod(disk_setup,10.d0) == 1.d0) .and. disk_ReadTable .and. first_lmax) then
   open(4200+myid,file='Boundary_table.txt',status='old')
   if (disk_TableLevel .eq. -1) disk_TableLevel = nlevelmax
   if ( (disk_setup .ge. 201) .and. (disk_setup .lt. 10001) ) then
     if (disk_setup .ge. 1001) then
       do i=1,2**(disk_TableLevel-1)
         read(4200+myid,*) P0_table(i), P1_table(i), P2_table(i), P3_table(i)
       enddo
     else
       do i=1,2**(disk_TableLevel-1)
         read(4200+myid,*) vrot_table(i), p_table(i), d_table(i)
       enddo
     endif
   endif
   close(4200+myid)
  ...
  do k=1,nregion
     ...
     if(region_type(k) .eq. 'point')then
        ...
        do i=1,nn
           ...
           enddo
#if USE_FLD==1
           T_region(k)=P_region(k)*mu_gas*mH/kb/d_region(k)*scale_v**2*r/vol
           do j=1,ngrp
