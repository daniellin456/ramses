recursive subroutine amr_step(ilevel,icount)
  ...
  if((hydro).and.(.not.static_gas))then
     ...
     ! Enforce velocity relaxation rules
     ! This can damp out velocity components at early times,
     !   or in geometric regions. It also should come in after all grav components. 
     if (apply_relaxation) then
