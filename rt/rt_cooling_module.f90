...
SUBROUTINE rt_solve_cooling(T2, xion, Np, Fp, p_gas, dNpdt, dFpdt        &
                           ,nH, c_switch, Zsolar, dt, a_exp, nCell,insink)
...
contains
  ...
  !-----------------------------------------------------------------------
    ...
    use const
    !use rt_cooling_module,only:rt_protostar_m1 !hybrid RT                                                
    use pm_commons !hybrid RT
    implicit none
    ...
    if(rt_protostar_m1)then
       ...
       if (nsink .gt. 0 .and. Teff_sink(1) .gt. 0.0d0  &
            & .and. nGroups==1) then
          kappaAbs(1) = planck_ana(rho, TK, Teff_sink(1), 1,insink(icell))/rho
       else
          kappaAbs(1) = 0.0d0 !no M1 photons if T(1)=0. Change it for multigrp
       endif
    endif
...
!!$          kSc_loc(iIR)  = kappaSc(iIR)  * (TR/10d0)**2 * exp(-TR/1d3)
       ...
       if(rt_protostar_m1) then !currently, use z_ave=1 in the namelist
          dustAbs(:)  = kAbs_loc(:) *rho*z_ave*rt_c_cgs
          dustSc(iIR) = kSc_loc(iIR)*rho*z_ave*rt_c_cgs
...
!-------------------------------------------------------------------------
  ...
  logical,dimension(1:nvector)::insink !for sink optically thin
!-------------------------------------------------------------------------
  aexp = astart
