...
#if NDUST>0
  real(dp),dimension(1:nvector,1:ndust,1:ndim),save::vdu
  integer :: idust
#endif  
...
#if NDUST>0
              !Gather dust drift velocity

              do idust=1,ndust
                 do idim=1,ndim
                    do i=1,ngrid
                       vdu(i,idust,idim)=v_dust(ind_cell_ref(i),idust,idim)
                    end do
                 end do
              end do
#endif      
...
#if NDUST>0
              !Gather dust drift velocity and ensure it's exaclty zero at the boundary (as grad_P=0 there)
              do idust=1,ndust
                 do idim=1,ndim
                    do i=1,ngrid
                       !v_dust(ind_cell(i),idust,idim) = 0.0d0! vdu(i,idust,idim)
                       if(no_inflow) then
                          if((boundary_dir.eq.1).or.(boundary_dir.eq.3).or.(boundary_dir.eq.5)) then
                             v_dust(ind_cell(i),idust,idim) = min(0d0,v_dust(ind_cell(i),idust,idim))
                          endif
                          if((boundary_dir.eq.2).or.(boundary_dir.eq.4).or.(boundary_dir.eq.6)) then
                             v_dust(ind_cell(i),idust,idim) = max(0d0,v_dust(ind_cell(i),idust,idim))
                          endif
                       endif
                    end do
                 end do
