...
#if NDUST>0
  real(dp),dimension(1:nvector,1:ndim),save::getxdust
  integer :: idust
#endif 
  ! Get child oct index
  do i=1,ncell
...
do idust=1,ndust
     ...
     do ind_son=1,twotondim
        ...
        do i=1,ncell
           do idim =1 , ndim
              getxdust(i,idim)=getxdust(i,idim)+v_dust(ind_cell_son(i),idust,idim)
           end do
        end do
     end do
     ! Scatter result to cells
     do i=1,ncell
        do idim=1,ndim
           v_dust(ind_cell(i),idust,idim) =getxdust(i,idim)/dble(twotondim)
        end do
     end do
  end do
  ! End loop over cell centered variables
