! ------------------------------------------------------------------------------
!  DUSTDIFF_SPLIT  This routine solves the dust source terms fluxes 
!  inputs/outputs
...
!
!  uin = (\rho eps_1, .... \rho eps_ndust,vdust ....,\rho,enint, rho dust_pscal1 ...)
! --------------------------------------------------------------------------
subroutine dustdiff_predict(uin,flux,dx,dy,dz,dt,ngrid,dv_ind)
  use amr_parameters
  use const             
  ...
  ! Input states
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust)::uin,q
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim),save::dq
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim),save::qm
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim),save::qp
  integer, dimension(1:ndust+1+ndust_pscal*ndust,1:ndim):: dv_ind
  
  ! Output fluxes
  real(dp),dimension(1:nvector,if1:if2,jf1:jf2,kf1:kf2,1:(ndust+1)+ndust_pscal*ndust,1:ndim)::flux 
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:(ndust+1)+ndust_pscal*ndust),save::Xflux,Yflux,Zflux
  real(dp)::flxmax,ddold,dold,dxdt
  ! Local scalar variables
  integer::i,j,k,l,ivar, idust,jdust,idust_pscal
...
!!$  !Put pscal under primitive form
  q=uin
!!$#if NDUST>0
!!$#if NDUSTPSCAL>0
!!$  do idust=1,ndust
...
!!$              do i = iu1, iu2
!!$                 do l = 1, ngrid
!!$                    q(l,i,j,k,ndust+(idust_pscal-1)*ndust+idust) = uin(l,i,j,k,ndust+(idust_pscal-1)*ndust+idust)/q(l,i,j,k,idust)
!!$                 end do
!!$              end do
!!$           end do
!!$        end do
...
!!$#endif
  ! Compute TVD slopes
  call uslope_dust(q,dq,dx,dt,ngrid)
  ! Compute 3D traced-states in all three directions
#if NDIM==1
     call trace1d_dust(q,dq,qm,qp,dx      ,dt,ngrid,dv_ind)
#endif
#if NDIM==2
     call trace2d_dust(q,dq,qm,qp,dx,dy   ,dt,ngrid,dv_ind)
#endif
#if NDIM==3
     call trace3d_dust(q,dq,qm,qp,dx,dy,dz,dt,ngrid,dv_ind)
#endif
  
 ! Solve for 1D flux in X direction
  call cmpflxmdust(qm,iu1+1,iu2+1,ju1  ,ju2  ,ku1  ,ku2  , &
       &       qp,iu1  ,iu2  ,ju1  ,ju2  ,ku1  ,ku2  , &
       &          if1  ,if2  ,jlo  ,jhi  ,klo  ,khi  , 2,3,4,Xflux,ngrid,dxdt,dv_ind)
  ...
  call cmpflxmdust(qm,iu1  ,iu2  ,ju1+1,ju2+1,ku1  ,ku2  , &
       &       qp,iu1  ,iu2  ,ju1  ,ju2  ,ku1  ,ku2  , &
       &          ilo  ,ihi  ,jf1  ,jf2  ,klo  ,khi  , 3,2,4,Yflux,ngrid,dxdt,dv_ind)
  ...
  call cmpflxmdust(qm,iu1  ,iu2  ,ju1  ,ju2  ,ku1+1,ku2+1, &
       &       qp,iu1  ,iu2  ,ju1  ,ju2  ,ku1  ,ku2  , &
       &          ilo  ,ihi  ,jlo  ,jhi  ,kf1  ,kf2  , 4,2,3,zflux,ngrid,dxdt,dv_ind)
...
subroutine uslope_dust(q,dq,dx,dt,ngrid)
  ...
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust)::q
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim)::dq
  ! local arrays
...
subroutine cmpflxmdust(qm,im1,im2,jm1,jm2,km1,km2, &
  ...
  integer ::ilo,ihi,jlo,jhi,klo,khi
  real(dp),dimension(1:nvector,im1:im2,jm1:jm2,km1:km2,1:nvardust,1:ndim)::qm
  real(dp),dimension(1:nvector,ip1:ip2,jp1:jp2,kp1:kp2,1:nvardust,1:ndim)::qp
  real(dp),dimension(1:nvector,ip1:ip2,jp1:jp2,kp1:kp2,1:(ndust+1)+ndust_pscal*ndust)::flx
  real(dp):: dxdt
  ! local variables
  integer ::i, j, k, l, xdim,idust,i0,j0,k0,idust_pscal
  real(dp)::entho
 ...
 !Dust ratio
  do k = klo, khi
     do j = jlo, jhi
        do i = ilo, ihi
           do idust =1,ndust+1+ndust*ndust_pscal

              do l = 1, ngrid
              qleft = qm(l,i,j,k,idust,xdim)
              qright = qp(l,i,j,k,idust,xdim)
              ! dust velocity
              uleft = qm(l,i,j,k,dv_ind(idust,xdim),xdim)
              uright = qp(l,i,j,k,dv_ind(idust,xdim),xdim)
              ! Compute fluxes
...
!!$            do idust_pscal=1,ndust_pscal
!!$            do l = 1, ngrid
!!$               qleft = qm(l,i,j,k,idust,xdim)*qm(l,i,j,k,ndust+(idust_pscal-1)*ndust+idust,xdim)
!!$               qright = qp(l,i,j,k,idust,xdim)*qp(l,i,j,k,ndust+(idust_pscal-1)*ndust+idust,xdim)
!!$               ! dust velocity
!!$               uleft = qm(l,i,j,k,dv_ind(idust,xdim),xdim)
!!$               uright = qp(l,i,j,k,dv_ind(idust,xdim),xdim)
!!$               ! Compute fluxes
!!$               call upwind_dust(uleft,uright,qleft,qright,flx(l,i,j,k,ndust+(idust_pscal-1)*ndust+idust))
!!$            end do
!!$         end do
!!$      end do
!!$      end do
...
!!$      do i = ilo, ihi
!!$            do l = 1, ngrid
!!$               qleft = qm(l,i,j,k,ndust+ndust_pscal*ndust+1,xdim)
!!$               qright = qp(l,i,j,k,ndust+ndust_pscal*ndust +1,xdim)
!!$               ! dust velocity
!!$               uleft = qm(l,i,j,k,dv_ind(ndust+ndust_pscal*ndust +1,xdim),xdim)
!!$               uright = qp(l,i,j,k,dv_ind(ndust+ndust_pscal*ndust +1,xdim),xdim)
!!$               ! Compute fluxes
!!$               call upwind_dust(uleft,uright,qleft,qright,flx(l,i,j,k,ndust+ndust_pscal*ndust +1))
!!$            end do
!!$      end do
!!$   end do
!!$end do
...
SUBROUTINE hlldust(vleft,vright,uleft,uright,fgdnv)
  ...
  if(0.le.vleft)  fgdnv = fleft
  if(0.ge.vright)  fgdnv = fright
...
!###########################################################
!###########################################################
subroutine trace1d_dust(q,dq,qm,qp,dx,dt,ngrid,dv_ind)
  use amr_parameters
  use hydro_parameters
  use const
  implicit none
  ...
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust)::q
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim)::dq
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim)::qm
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvardust,1:ndim)::qp
...
!###########################################################
#if NDIM>1
subroutine trace2d_dust(q,dq,qm,qp,dx,dy,dt,ngrid,dv_ind)
  use amr_parameters
  use hydro_parameters
  use const
  implicit none
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              rhod =  q(l,i,j,k,idust)
              ud   =  q(l,i,j,k,dv_ind(idust,1))
              vd   =  q(l,i,j,k,dv_ind(idust,2))
              ! TVD slopes in all 3 directions
              drhox = dq(l,i,j,k,idust,1)
              dux =   dq(l,i,j,k,dv_ind(idust,1),1)
              dvx =   dq(l,i,j,k,dv_ind(idust,2),1)
              drhoy = dq(l,i,j,k,idust,2)
              duy =   dq(l,i,j,k,dv_ind(idust,1),2)
              dvy =   dq(l,i,j,k,dv_ind(idust,2),2)
              ...
              qp(l,i,j,k,dv_ind(idust,1),1) = ud     - half*dux   !+ su0*dtdx*half
              qp(l,i,j,k,dv_ind(idust,2),1) = vd     - half*dvx  ! + sv0*dtdx*half
              ! Left state at right interface
              qm(l,i,j,k,idust,1) = rhod       + half*drhox + srho0*dtdx*half
              qm(l,i,j,k,dv_ind(idust,1),1) = ud     + half*dux   !+ su0*dtdx*half
              qm(l,i,j,k,dv_ind(idust,2),1) = vd     + half*dvx   !+ sv0*dtdx*half
              ! Top state at bottom interface
              qp(l,i,j,k,idust,2) = rhod       - half*drhoy + srho0*dtdy*half
              qp(l,i,j,k,dv_ind(idust,1),2) = ud     - half*duy   !+ su0*dtdy*half
              qp(l,i,j,k,dv_ind(idust,2),2) = vd     - half*dvy   !+ sv0*dtdy*half
              ! Bottom state at top interface
              qm(l,i,j,k,idust,2) = rhod       + half*drhoy + srho0*dtdy*half
              qm(l,i,j,k,dv_ind(idust,1),2) = ud     + half*duy   !+ su0*dtdy*half
              qm(l,i,j,k,dv_ind(idust,2),2) = vd     + half*dvy   !+ sv0*dtdy*half
              end do
           end do
        end do
     end do
...
!###########################################################
#if NDIM>2
subroutine trace3d_dust(q,dq,qm,qp,dx,dy,dz,dt,ngrid,dv_ind)
  use amr_parameters
  use hydro_parameters
  use const
  implicit none
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           do l = 1, ngrid
              ...
              ud   =  q(l,i,j,k,dv_ind(idust,1))
              vd   =  q(l,i,j,k,dv_ind(idust,2))
              wd   =  q(l,i,j,k,dv_ind(idust,3))
              ! TVD slopes in all 3 directions
              drhox = dq(l,i,j,k,idust,1)
              dux = dq(l,i,j,k,dv_ind(idust,1),1)
              dvx = dq(l,i,j,k,dv_ind(idust,2),1)
              dwx = dq(l,i,j,k,dv_ind(idust,3),1)
              drhoy = dq(l,i,j,k,idust,2)
              duy = dq(l,i,j,k,dv_ind(idust,1),2)
              dvy = dq(l,i,j,k,dv_ind(idust,2),2)
              dwy = dq(l,i,j,k,dv_ind(idust,3),2)
              drhoz = dq(l,i,j,k,idust,3)
              duz = dq(l,i,j,k,dv_ind(idust,1),3)
              dvz = dq(l,i,j,k,dv_ind(idust,2),3)
              dwz = dq(l,i,j,k,dv_ind(idust,3),3)
              ...
              qp(l,i,j,k,dv_ind(idust,1),1) = ud - half*dux   !+ su0*dtdx*half
              qp(l,i,j,k,dv_ind(idust,2),1) = vd - half*dvx  ! + sv0*dtdx*half
              qp(l,i,j,k,dv_ind(idust,3),1) = wd - half*dwx   !+ sw0*dtdx*half
              ! Left state at left interface
              qm(l,i,j,k,idust,1) = rhod       + half*drhox + srho0*dtdx*half
              qm(l,i,j,k,dv_ind(idust,1),1) = ud + half*dux  ! + su0*dtdx*half
              qm(l,i,j,k,dv_ind(idust,2),1) = vd + half*dvx  ! + sv0*dtdx*half
              qm(l,i,j,k,dv_ind(idust,3),1) = wd + half*dwx  ! + sw0*dtdx*half
              ! Top state at bottom interface
              qp(l,i,j,k,idust,2) = rhod       - half*drhoy + srho0*dtdy*half
              qp(l,i,j,k,dv_ind(idust,1),2) = ud - half*duy  ! + su0*dtdy*half
              qp(l,i,j,k,dv_ind(idust,2),2) = vd - half*dvy  ! + sv0*dtdy*half
              qp(l,i,j,k,dv_ind(idust,3),2) = wd - half*dwy  ! + sw0*dtdy*half
              ! Bottom state at top interface
              qm(l,i,j,k,idust,2) = rhod       + half*drhoy + srho0*dtdy*half
              qm(l,i,j,k,dv_ind(idust,1),2) = ud + half*duy  ! + su0*dtdy*half
              qm(l,i,j,k,dv_ind(idust,2),2) = vd + half*dvy  ! + sv0*dtdy*half
              qm(l,i,j,k,dv_ind(idust,3),2) = wd + half*dwy   !+ sw0*dtdy*half
              ! Back state at front interface
              qp(l,i,j,k,idust,3) = rhod       - half*drhoz + srho0*dtdz*half
              qp(l,i,j,k,dv_ind(idust,1),3) = ud - half*duz   !+ su0*dtdz*half
              qp(l,i,j,k,dv_ind(idust,2),3) = vd - half*dvz  ! + sv0*dtdz*half
              qp(l,i,j,k,dv_ind(idust,3),3) = wd - half*dwz  ! + sw0*dtdz*half
              ! Front state at back interface
              qm(l,i,j,k,idust,3) = rhod       + half*drhoz + srho0*dtdz*half
              qm(l,i,j,k,dv_ind(idust,1),3) = ud + half*duz  ! + su0*dtdz*half
              qm(l,i,j,k,dv_ind(idust,2),3) = vd + half*dvz  ! + sv0*dtdz*half
              qm(l,i,j,k,dv_ind(idust,3),3) = wd + half*dwz  ! + sw0*dtdz*half
              end do
           end do
        end do
     end do
